...
> **Status 2026-09-23: Question 1 answered, Questions 2, 3 and 4 still open.** Gate 0c remains
> **blocked** — the instrument list alone does not release it, because steps 6, 11, 12 and 14a each need
> values this document has not yet received (steps 7 and 8 are filtered to the instruments, and wait on
> the gate too). See the answer block under Question 1.
>
...
**What this is.** Four decisions that only you can make. Everything else in the NY_SCH BOX FE
...
> those numbers.*
>
> ⚠️ **For information: NY's GBO holds `0` for all six.** Run 3 used those zeros as a test decision.
> Zero is the value this question exists to avoid, and it is not what we propose.
...
**Every instrument also gets a dummy-book row** `[stated: BOX FE expert via Edouard, 2026-09-23]` —
one per (book × instrument) **plus** one per (dummy book × instrument); dummy label `26391.4` in Tier 1.
You are signing the real books only; the dummy row is added for every instrument regardless. For one
book, that is **12 rows** across six instruments.

⚠️ **The dummy label itself is not your decision** — Tier 1's `26391.4` does not exist in Tier 2, and
which label replaces it is a question for the BOX FE team (Q-10d). It cannot be waived, and it is never
...
## Question 4 — Days Matured, for instruments that have none *(new 2026-09-23)*

`T_BOX_ENGDAYS_MATURED_S` holds `NUM_DAYS` **per instrument, for all branches**. `[stated: BOX FE expert
via Edouard, 2026-09-23]` If one of your six instruments already has a row, nothing changes. If it has
none, we add one, **configured as London has it** in Tier 1.

| Instrument | Row in Tier 2? | London (Tier 1) `NUM_DAYS` | NY_SCH | |
|---|---|---|---|---|
| *(only instruments without a row — filled from Q-12)* | ☐ none | | | ☐ accept ☐ change to ____ |

`DDATE` is proposed as the date the script runs (`SYSDATE`), as the BOX FE team writes it. ☐ agree ☐ other: ____

**Sign-off:** ☐ as proposed  ☐ as amended  ☐ not needed — every instrument already has a row
...
| | |
|---|---|
| Blocks | walk steps **6** (Accrual), **7–8** (exceptions, filtered to the instruments), **11** (Book), **12** (Allowed Errors), **14a** (Days Matured) — and therefore the findings table as a whole |
