...
**Status:** adopted 2026-09-23 · point 3's column question **settled 2026-09-24: `FK_BS`** (see *After run 4*)
...
| 3 | Don't list the quote references in the file — pick them with `SELECT DISTINCT PK FROM DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = 1.35` — *"not sure the values … were correct"* | **Yes on typing them.** **Open on the column** — the expert's query copies `PK`, the repo's confirmed join copies `FK_BS` | Step 4's list is **generated** from Q-04c's CSV (`scripts/evidence_to_sql.py`), length-guarded in the script; validator checks SQL = CSV; verify V4 checks BOX = GBO; **Q-04c decides the column and blocks step 4 until it runs** | `quote-refs-not-generated`, `quote-refs-not-evidence` |
...
## After run 4 — 2026-09-24

- **Point 3, settled: `FK_BS`.** `[stated: operator, BOX Dev, 2026-09-24]` — `T_PGT_ENGLKFC_X.FK_BS =
  T_PGT_QUOTE_REFERENCE_S.PK`. The side-by-side Q-04c that stopped run 4 was a flawed test: it treated a
  bridge PK that happened to equal a quote-reference PK (`87.35`) as evidence. Q-04c is now the decoded,
  export-ready set.
- **Point 5: Tier 1's dummy label `26391.4` does not exist in Tier 2.** New Q-10d finds candidates; the
  label is a **BOX FE decision** on a card, the books a product-SME card. The dummy row is never waived and
  never a real book's label.
- **Decisions are asked one card at a time** and logged in `00-decisions.md`.
- Review: [`REVIEW-run-04.md`](../../runs/NY_SCH/tier2-pre/03-sql/REVIEW-run-04.md).
...
- **The config script touches `BOX_FE` only** `[assumed: Edouard, 2026-09-23]` — the applying account
  sees nothing else. Sets mined from GBO (steps 4, 7) are literals **generated from the evidence CSVs** by
  `scripts/evidence_to_sql.py`; the validator checks them against the same CSVs, and the verify script —
  run on the operator's read-only account — checks BOX against GBO (V4, V10). An earlier draft of this
  change read `DEVENG` inside the script; withdrawn the same day. Check `config-reads-other-schema`.
