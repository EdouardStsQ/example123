...
| 5 | **Jobs — BOX_FE/BOX_ACC internal batch analogue.** Review the BOX_FE / BOX_ACC internal batch jobs (Control-M / MBJ), general and product-specific, an existing branch runs for a given product, and create the equivalent for the new branch — e.g. "if Madrid Deposits runs this job sequence, NY Deposits probably needs the same sequence." | same | Control-M / MBJ, BOX_FE/BOX_ACC internal batch | `[stated]` **No longer a sketch as of 2026-09-16.** Both layers are now documented: `../reference/job-chains/box-fe-acc-batch-runtime.md` gives the Control-M job topology for FE *and* ACC from repository configuration (upgrading the five repo names in `repo-index.md` from hearsay to confirmed), and `../reference/job-chains/fe-batch-event-groups.md` gives the FE event-group sequence the jobs dispatch into. What remains open is not "what does the topology look like" but "which of it a *new branch* needs" — every observation is Tier 1 / Madrid. |
