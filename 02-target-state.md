...
|---|---|---|
| 1. Configs — look up GBO's FE+ACC config | `branch-config-agent`, Phase 1 (mine) | Once Tier 2 DB access exists, this becomes a deterministic query (`branch-config-surface.md` §5's mining method) instead of manual SIGOM screen reading. |
| 2. Configs — adapt to BOX schema | `branch-config-agent`, Phase 1 (propose) / Phase 3 (apply) | Proposal is agent-driven for structure; GL account values and portfolio-property breakdowns stay SME-owned per the golden rule — never invented. See the "never writes to GBO or BOX" guardrail in `agents/branch-config-agent/AGENT.md`. |
...
| 5. Jobs — BOX_FE/BOX_ACC internal batch analogue | `branch-config-agent`, Phase 1 | **Unblocked, both engines (2026-09-16).** Two documents now cover this between them: `../reference/job-chains/box-fe-acc-batch-runtime.md` (Control-M topology, FE **and** ACC, from repository configuration) and `../reference/job-chains/fe-batch-event-groups.md` (the FE event-group sequence the jobs dispatch into). So the step is no longer "propose from analogue" but "reproduce a known topology for the new branch". The residual risk moved: every observation is **Tier 1 / Madrid**, and the Tier 2 equivalents of those five repos have never been identified — see the open question in `../reference/repo-index.md`. |
...
Every step above becomes either an **agent** (`box-datalake-expert`, `branch-config-agent`, orchestrating via `branch-onboarding-orchestrator`) or **stays manual, with a reason**: deployment-to-PRE (mechanics not yet understood) and Phase 2 sign-off (manual by design, not a gap to close).
