...
**Runs between mining and findings review, and only when `GBO_SOURCE` and `TARGET_ENV` are in
different tiers.** Every FK value mined from the source is a PK *in the source tier*; before an
`INSERT` carries one into the target, it has to resolve there — to the *same object*.
