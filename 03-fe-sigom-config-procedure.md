...
| E7 | **Call each object's pre-commit procedure** after its INSERT, with the PK variable just allocated — charter hard rule 10. Affects steps 2, 3, 4, 5, 6 *(added 2026-09-18)* |
...
- ✅ **F3 now has a method** *(2026-09-18)*. Five walk objects declare a `PRE_COMMIT_PROC`, so the
  comparison is no longer freeform: in a reference environment, inside one transaction — snapshot,
  INSERT, call the procedure, re-snapshot, `ROLLBACK`. Any change beyond your own row is what SIGOM does
  and your script does not. Read the procedure bodies from `cib-boxfin-dbboxfe` first;
  `ALL_SOURCE` shows package specs only.
