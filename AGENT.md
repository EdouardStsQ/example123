...
> | Affected | Change |
> |---|---|
> | `PRODUCT_BOOK_SCOPE` | Instruments are chosen from **BOX's live catalogue**, by a named SME — not derived from the branch's GBO instrument configuration. Two BOX-side tables answer two different questions: the **menu** (every instrument BOX has) is `T_BOX_ENGINSTRUMENTS_S`, the Processed Instruments catalogue in [`box-data-model.md`](../../docs/reference/box-data-model.md); **which instruments a given branch actually has** is the **Accrual tab**, `T_BOX_ENGACCRCONF_S`, one row per instrument. See *Reading a branch's instrument set* below |
...
8. **Before recording an absence, prove the query could have returned a presence.** A zero-row result
   ...
   **This rule is written from two real incidents, both in 2026-09-16's first live run.** Q-G2 levels
   2–3 returned nothing for NY_SCH and were recorded as a real absence — then returned nothing for
   Madrid too, proving the join wrong. And Q-G4 reported Tier 2 PRE as missing a substantial number of
   `BOX_FE` tables — from an account with no grants on that schema, so it had measured visibility, not
   existence. Both conclusions were confidently wrong and both were cheap to prevent. Four catalogue
   queries still rest on an assumed join column (see the catalogue's Coverage check); treat every one
   of them as a live instance of this rule.
...
| 0e | **Target schema complete** — *in the environment this run targets* | Q-G4 |
...
Two BOX-side tables, two different questions — do not substitute one for the other:

| Question | Table | Shape |
|---|---|---|
| What instruments does **BOX** have at all? — the menu | `T_BOX_ENGINSTRUMENTS_S` (Processed Instruments) | Global catalogue, 18 rows, no branch dimension |
| What instruments does **this branch** have? — the selection | `T_BOX_ENGACCRCONF_S` (Accrual tab) | One row per instrument, per MIS configuration |
...
1. **Gate 0c becomes verifiable.** "NY_SCH gets the same set as SLB" stops being an unfalsifiable SME
   ...
   than a phrase, and a reference branch's set can be put in front of them.
...
### Rehearsal mode — when gate 0d is unresolved but the walk still needs testing
...
In rehearsal mode the agent emits every primary key as a **named substitution variable**, never a
...
graph explicit and auditable, which is the main thing a rehearsal is testing. The file is deliberately
not runnable until a human binds the DEFINEs, and says so in its header. The moment 0d resolves, the
same file becomes runnable by filling in that block — no regeneration.

What does not relax: no invented literal, no sequence that has not been mapped to its table, no PK
copied from an existing row, and every other gate and hard rule applies unchanged. A rehearsal run is
recorded with its own `RUN_FOLDER` (e.g. `runs/<branch>/tier1-rehearsal/`), and every output file
carries a header naming the environment its **values** came from and stating that they must not be
carried to another tier. Values never travel between environments — hard rule 6 and 7 — and a
rehearsal produces a validated *shape*, never a draft to promote.
