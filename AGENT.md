...
10. **The INSERT is not the whole operation — call the pre-commit procedure.** `[confirmed: DB,
    2026-09-18]` SIGOM objects can declare `PRE_COMMIT_PROC` / `FK_PRECOMMIT`, and **five of the walk's
    objects do**, covering steps 2, 3, 4, 5 and 6:
    ...
    executing account needs no extra grants. **After each affected INSERT, the script calls the
    procedure with the PK variable it already holds.** If the procedure validates, the script reproduces
    SIGOM's check; if it writes, the script reproduces the write. Either way the gap closes.
