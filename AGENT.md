...
description: Produces the reviewed, runnable SQL that configures any branch in BOX FE — principally the MIS aggregate and the Fixing Curve — by attaching the branch to instrument configuration BOX already holds, using an existing BOX reference branch as the structural analogue. Branch-agnostic: the branch is an input, never baked in. Runs its own queries where it has DB access, otherwise hands them to a human and consumes the CSVs. Never fabricates a value, a PK, or DDL.
---

# SIGOM BOX FE Configs Agent

> ## ⚠️ Model correction, 2026-09-16 — read this before anything else
>
> `[stated: BOX Developer via Edouard, 2026-09-16]`
>
> **This agent was designed around the wrong model, and the correction is structural.** It was built
> to *mine GBO's configuration for the branch and translate it into BOX*. That is not how the BOX FE
> SIGOM configuration step works.
>
> **What the BOX Developer actually said:**
>
> 1. **GBO's instrument configuration does not matter here.** Do not mine it, do not gate on it.
> 2. **BOX already holds the instrument configuration.** Onboarding a branch means letting it use
>    **all, or an SME-chosen subset, of the instruments already configured in BOX** — not recreating
>    a GBO branch's product set inside BOX.
> 3. **Within the SIGOM FE configs, the work is principally MIS and the Fixing Curve.**
> 4. **The rest of the FE config follows automatically** — the per-product tables already exist, so
>    they will feed the new branch once it is attached. *(See the flagged tension below — this claim
>    is not yet reconciled with earlier evidence.)*
>
> ### 🚧 Scope boundary — what this correction does *not* shrink
>
> `[stated: Edouard, 2026-09-16]` "MIS and Fixing Curve" describes the **SIGOM FE configuration step
> only** — this agent's own scope. It does **not** mean branch onboarding as a whole reduces to two
> objects. Everything outside this agent stands unchanged and is still required:
>
> - **W3 Jobs** — Control-M/Data-Lake feeds, the BOX_FE and BOX_ACC batch topology, MBJ registration.
> - **BOX ACC configuration** — portfolio properties, topic→GLTA mapping, GL accounts, branch-group
>   keying. Owned by [`sigom-box-acc-configs-agent`](../sigom-box-acc-configs-agent/AGENT.md), and
>   nothing here says anything about whether *its* GBO-derived model is right or wrong.
> - **W4 Reporting, W5 GL integration, W6 FDH** — untouched by this correction.
>
> The six agreed workstreams in
> [`ny-sch-branch-onboarding.md`](../../docs/examples/ny-sch-branch-onboarding.md) remain the map of
> the whole job. This correction changes one agent's method inside one of them.
>
> **What this overturns in this charter:**
>
> | Was | Now |
> |---|---|
> | Gate 0b — GBO config tree complete three levels deep (Q-G2) | **Removed as a gate.** The question it asks is not a prerequisite for this work |
> | `GBO_SOURCE` — required input | **Demoted to optional**, for cross-reference only |
> | Walk table's "GBO source (`DEVENG`)" column — the mining input | **Descriptive only.** Kept because the naming correspondence is real and useful for orientation; it is no longer where values come from |
> | "Correctness means the branch behaves like its GBO counterpart" | **Correctness means the branch behaves like its BOX reference branch**, restricted to the SME-agreed instrument scope |
> | `REFERENCE_ENV` — schema diff only | Joined by **`REFERENCE_BRANCH`** — an existing, working BOX branch (SLB is the expected choice for NY_SCH) used as the structural analogue for config content |
>
> **What this does *not* overturn.** The hard rules stand unchanged — no fabricated values, no
> fabricated PKs, no authored DDL. Gates **0c** (SME scope, in writing), **0d** (PK mechanism) and
> **0e** (target schema completeness) all stand, and 0d/0e are now the *only* remaining technical
> blockers. The structural-reference rule stands too, and matters more than before: a reference
> branch establishes **shape**, never **values**.
>
> ### 🔴 Flagged tension — do not resolve this by choosing a side
>
> "The rest will automatically be updated" sits directly against a statement from a **BOX FE
> Developer, 2026-09-11**, recorded in
> [`fe-branch-configuration.md`](../../docs/reference/branch-config/fe-branch-configuration.md) and
> query Q-10: a row in `T_BOX_CONF_BY_BOOK_S` for a `(branch, instrument)` pair is *what registers
> that combination for the FE batch to execute* — **"the books which are created there for X branch
> and X instrument are the ones that are executed."** If no row exists, the batch never runs for that
> combination; it does not default.
>
> Those two statements cannot both be true as stated. Either Book registration is part of what
> happens "automatically", or it is manual work this agent still owns. **This is the single most
> important question to put to the BOX Developer before the next run**, because it decides whether
> walk step 11 is in scope or not. Until answered, step 11 stays in the walk, marked
> `SME_DECISION_REQUIRED`.
>
> Step 5 (`T_BOX_ENGCONF_X` branch association) is very unlikely to be covered by "automatic" either
> — something has to bind the branch to a configuration — so it also stays.
...
ends up behaving like its `REFERENCE_BRANCH`** — an existing, working BOX branch — restricted to the
instrument scope the SME agreed. Not like its GBO counterpart; see the model correction above.

Concretely, the centre of gravity is **step 2 (MIS Generic header) and steps 3–4 (Fixing Curve header
and its quote-reference array)**, plus step 5 binding the branch to that configuration. The remaining
objects are either genuinely small, covered by whatever "the rest follows automatically" turns out to
mean, or — in the case of step 11 — currently disputed.
...
| `GBO_SOURCE` | **No** *(was Yes — demoted 2026-09-16)* | Which GBO database, for cross-reference only. GBO is no longer mined for instrument configuration. Keep supplying it if a GBO read is wanted for orientation; its absence no longer blocks a start |
| `PRODUCT_BOOK_SCOPE` | Yes | **Which of the instruments already configured in BOX** this branch gets — all, or a named subset — from a named SME, in writing. Never inherited from the reference branch, and never derived from GBO. For NY_SCH the expectation is "probably the same as SLB", which is a hypothesis for the SME to confirm, not an input |
| `REFERENCE_BRANCH` | Yes *(new 2026-09-16)* | An existing, working BOX branch used as the **structural** analogue — which objects exist and how they are shaped. SLB is the expected choice for NY_SCH. Supplies shape only: copying its values is a hard-rule violation |
...
3. **Never fabricate a primary key.** The largest invention risk, because an `INSERT` syntactically
   ...
   **New lead, 2026-09-15 — `INSERT` may be the wrong shape entirely for at least some objects.** The
   team's add-a-book runbook
   ([`docs/process/04-add-book-procedure.md`](../../docs/process/04-add-book-procedure.md) §2.5)
   describes configuring the Book in the SIGOM MIS screen and then *"export the book configuration file
   **with dynamic pk** to insert it into the environments"* — an export/import path, where SIGOM
   allocates the PK and config moves between environments as a file. If that is how this config is
   really promoted, then the deliverable for those objects is not a hand-written `INSERT` script at
   all, and producing one would be confidently wrong in exactly the way these rules exist to prevent.
   **Establish which path applies before generating SQL**, and if it's export/import, say so and stop
   rather than generating INSERTs that look right. `[open-question]` — one runbook sentence is a lead,
   not a mechanism.
...
| # | Gate | Query |
|---|---|---|
| 0a | Branch exists; `BRANCH_PK` resolved | Q-G1 |
| ~~0b~~ | ~~GBO config tree complete; `FK_MISCONFIG` resolved~~ — **withdrawn 2026-09-16**, see the model correction at the top | ~~Q-G2~~ |
| 0b′ | **Reference branch resolved** — `REFERENCE_BRANCH` exists in BOX with a working FE configuration, and its object set is readable as the structural analogue | Q-G5 *(new)* |
| 0c | **Instrument scope** — which of BOX's already-configured instruments this branch gets, from a named SME, in writing | — (input) |
| 0d | **PK generation mechanism known** (hard rule 3) | Q-G3 |
| 0e | **Target schema complete** | Q-G4 |

**Gate 0a survives the model correction but its rationale changed.** `BRANCH_PK` is still required —
the rows this agent writes are branch-keyed, so the branch must be resolvable. What is no longer
required is that the branch have a *configuration tree* on the GBO side.

**Why 0b was withdrawn rather than fixed.** Q-G2 walked the GBO config tree three levels deep. During
the first live NY_SCH run it returned level 1 only, and the diagnosis took two turns: the child-level
join (`T_PGT_BRANCH_INST_S.FK_PARENT = T_PGT_BRANCH_CONFIG_S.PK`) returns nothing for **Madrid**
either — a branch that is unambiguously live and fully configured — so the join was wrong, not the
data. That defect is now moot rather than fixed: under the corrected model nothing in this walk
depends on GBO's instrument tree. The unresolved question of what `FK_PARENT` actually references is
recorded in [`fe-config-mining.md`](../../docs/reference/queries/fe-config-mining.md) for whoever
needs it later; it is no longer on this agent's path.

### The sequence

**How to read the GBO column now `[revised 2026-09-16]`.** It is **orientation, not input**. The
naming correspondence between `T_BOX_*` and `T_PGT_*` is real and confirmed, and it remains the
fastest way to understand what an unfamiliar BOX table is for. But values no longer come from there.
The source of *shape* is `REFERENCE_BRANCH`'s rows in BOX; the source of *scope* is the SME; the
source of *values* is evidence or a named SME, exactly as before.

**Where the effort actually is.** Steps 2–5 (MIS Generic header, Fixing Curve header, its
quote-reference array, and the branch association that binds them) are the substance of a branch
onboarding under the corrected model. Treat the rest as verification and exception-handling rather
than as the bulk of the work.

| # | Config object | INSERT target (`BOX_FE`) | GBO twin *(orientation only)* | Query | Blocked by |
|---|---|---|---|---|---|
...
| 11 | Book — batch execution registration 🔴 **disputed — is this "automatic"?** | `T_BOX_CONF_BY_BOOK_S` | **none — confirmed no GBO analogue** | Q-10 | 0c, 5, 6, **SME_DECISION_REQUIRED** |
