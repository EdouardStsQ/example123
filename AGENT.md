...
| 0f | **Cross-tier FK resolution** — *split-tier runs only.* Every mined FK value that will appear in an `INSERT` resolves in `TARGET_ENV`. Evaluated **after** mining, before SQL | Q-14 |
...
### Gate 0f — cross-tier FK resolution

`[new 2026-09-18]` **Only applies when `GBO_SOURCE` and `TARGET_ENV` are in different tiers**, and it
is the one gate evaluated *after* mining rather than before the walk — it cannot be, since it tests
values the walk produces.
...
### Split-tier runs — mining one tier, writing another
