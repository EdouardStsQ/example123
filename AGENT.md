...
|---|---|
| W2 SIGOM configs (FE + ACC) | FE: `sigom-box-fe-configs-agent` · ACC: `branch-config-agent` (interim — see *Agents and skills it may call*) |
...
| `sigom-box-fe-configs-agent` | **The BOX FE walk** — the 14-step gated config walk producing runnable FE SQL (gates 0a–0e, its own kickoff prompt, its own run folder). This is what executes the FE half of W2 today |
| `branch-config-agent` | BOX_ACC config (portfolio properties, topic→GLTA, GL accounts) mined from GBO Tier 2, **and the mining/proposal skills** (`mine-fe-branch-config`, `mine-branch-config`, `propose-branch-config`) that the FE agent calls |

**Interim routing — read before delegating W2.** `branch-config-agent` and `sigom-box-fe-configs-agent`
overlap on BOX FE config, and which one owns it is an open decision for the BOX Lead (see
[`agent-architecture.md`](../../docs/reference/branch-config/agent-architecture.md)'s "The three agents"
section). **Until it is decided, the FE walk runs via `sigom-box-fe-configs-agent` and its kickoff
prompt** — it is the one built to be run next, and it carries gates 0d/0e and the SQL deliverable that
`branch-config-agent`'s sequence does not. `branch-config-agent` supplies the skills it calls. Do not
run both FE sequences.
...
> **Branch-group resolution is load-bearing, not a checklist item** `[confirmed: DB, 2026-09-21]`. The
> metamodel declares that **`BOX - Acct Port Property` and `BOX - Config LOC Property` key on the branch
> *group*** (`T_PGT_BRANCH_GROUP_S`), while twelve other ACC objects key on the branch — **through a
> field named `pBranch` in both cases**. For NY_SCH that is `FK_LOCALGROUP = 21447.4` versus
> `BRANCH_PK = 20007.4`; both are valid PKs and the wrong one fails silently. So Phase 0's group
> resolution is a hard prerequisite for the ACC half, and a branch joining an existing group may need
> no INSERTs at all for those two objects. See
> [`sigom-box-acc-configs-agent/AGENT.md`](../sigom-box-acc-configs-agent/AGENT.md).

> **⚠️ An operational onboarding step nobody had recorded** `[confirmed: source via Devin, 2026-09-21]`.
> A new branch's **processing calendar** is not configuration — it is *generated* by
> `PKG_BOXTOOLS.p_Scheduling_Monitor(P_numdays, P_ddate, P_branch_id, P_instrument_id, P_calendar_id)`,
> which inserts a `T_BOX_BRPROCCAL_S` row per working day, reading `T_BOX_ENGCONF_S`/`T_BOX_ENGCONF_X`
> (joined on `FK_BS = P_branch_id`) to pick the config and curve.
>
> **Someone has to run it for the new branch.** It sits outside the FE config walk — the FE agent
> correctly never writes `T_BOX_BRPROCCAL_S` — but the branch has no processing schedule until it runs,
> and nothing in this repo's checklists owns that action. **Assign it.** Same for
> `PKG_ENGGNCP_OPTZ.p_create_process_queues_optz`, which builds the queues and likewise depends on the
> step-5 branch-association row existing.
>
> Not an issue: `PKG_BATCHPROCESS_MBJ.p_ReadLog` self-seeds a branch's logging rows in `PGT_PRC` on
> first use, so logging needs no onboarding action.
>
> **There is no "provision a new branch" script.** Devin searched both repos: the core `_S` config
> tables have no `INSERT` anywhere. Base configuration is entered through the SIGOM UI; what the repos
> automate is expanding a portfolio's topic/condition lists (ACC) and generating the branch/instrument
> processing calendar and queues (FE). So the walk this repo designed has no authoritative counterpart
> to be checked against — it *is* the specification.
...
| An external doc arrives (e.g. `box-fe-acc-batch-runtime`, which landed 2026-09-16) | Import it, resolve the de-linked citations that reference it, add its row to `docs/topic-index.md`. **Check the filename is not already reserved by a citation** before writing anything to it — that exact name was carried as *pending — not yet imported* in eight places, and a second document was written into it by mistake. If two documents genuinely cover the same layer from different evidence, keep both under distinct names and cross-link them rather than merging (see `fe-batch-event-groups.md`) |
...
- **Success:** all **five** Definition of done criteria met and SME-accepted — criterion 5 included. Criteria 1–4 passing means the branch works, not that the job is done.
