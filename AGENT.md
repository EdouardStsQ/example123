...
| `APPROVED_INSTRUMENTS` | Yes, once 0c's instruments are known | Gate 0c's instrument answer **as PKs**, one line in `00-inputs.md`. The validator checks every instrument-keyed step against it — names are not checkable |
| `SOURCE_FRONT` | Yes, for step 2 | `T_PGT_SOURCE_S` PK of the **Murex instance** the branch's trades come from, from a named person (Q-02c lists them). NY: `513.4`, Murex 3 Latam. `FK_SOURCE_BACK` is not an input — it is always `586.4` |
| `DUMMY_BOOK_LABEL` | Yes, for step 11 | The dummy-book label every instrument gets a row for. Tier 1 `26391.4` **does not exist in Tier 2** — found by **Q-10d** and decided by the BOX FE team on a card. **Never a real book's label** |
| `TARGET_AUTH_CODE` | Recorded, not supplied | Read by **Q-G3c** and written into `00-inputs.md` with its PK fraction (`TARGET_PK_FRACTION`). The config script aborts if the environment it runs in has a different one |
| `QUOTE_REF_COLUMN` | Fixed | `FK_BS` — settled 2026-09-24 (`FK_BS = T_PGT_QUOTE_REFERENCE_S.PK`) |
| `REFERENCE_BRANCH_PK` | Yes | The reference branch's PK in `PGT_STC.T_PGT_BRANCH_S` (NY_SCH: SLB, `20087.4`). The only branch PK a reference query uses |
...
| 0 | `00-decisions.md` | One row per decision card answered — decision, value, name, role, date, evidence |
...
| 3 | `03-sql/` | **`values.json`, written by the agent; four `.sql` files RENDERED from it** `[2026-09-24]` by `scripts/render_sql.py` — `<BRANCH>-<env>-rehearsal.sql` (the config block ending in `ROLLBACK`), `-config.sql` (the same block ending in the committing pre-commit), `-verify.sql` (read-only checks, auth code included), `-rollback.sql` (the only undo) — plus `render-report.md`. **The agent never writes or edits a `.sql` file**; see *The SQL is rendered, never written* below |
...
### The SQL is rendered, never written `[2026-09-24]`

Runs 1–5 hand-wrote the SQL from the templates, and each broke its structure somewhere new — a cursor
field that did not exist, a procedure inside `VALUES`, guards that could not fire, a list declared and
never used ([ADR 0006](../../docs/decisions/0006-render-sql-from-values.md)). The agent's judgement is
needed for the **values** and the **decisions**, not for PL/SQL syntax. So:

1. The agent writes **`03-sql/values.json`** — every value with its `source` (a query ID, an input, or
   `decision N`), from [`templates/values.example.json`](templates/values.example.json).
2. It runs **`python3 scripts/render_sql.py <RUN_FOLDER>`**. The renderer cross-checks the values against
   `00-inputs.md`, `00-decisions.md`, `02-findings.md`, the evidence CSVs and the target's columns
   (Q-G3d), and **refuses** — naming the JSON path and the fix — anything it cannot stand behind. Then it
   writes the four files and runs the validator.
3. A refusal is fixed **in the values or the evidence**, never in the SQL. The validator re-renders and
   FAILs any `.sql` file that differs (`sql-edited-by-hand`), and any file it did not produce. If the
   rendered SQL looks wrong, that is a template defect: say so, and stop.

The templates carry everything below — the step headers, the pre-flight guards, the auth-code check,
the self-check, the pre-commits — so these sections describe what the rendered files contain, not what
the agent types.
...
-- Depends on  : its PK, v_curve_pk, is allocated in step 2 — the header already points at it
...
**Ask for one query per message** `[stated: operator, 2026-09-24]`, in dependency order. Gate queries
first — Q-G1's result is required to parameterise everything else, and Q-G3/Q-G4 can stop the run
entirely. **Every parameter value in a query names its source** (`REFERENCE_BRANCH_PK 20087.4 —
input`); a PK taken from the wrong list is how run 5 asked for SLB's error limits with a book label as
`FK_BRANCH`.
...
3. **Never fabricate a primary key — call `F___SEQUENCE`.** `[confirmed: source, 2026-09-17]`
   ...
   **The auth code must be visible and asserted, not merely present** `[stated: BOX FE expert via
   Edouard, 2026-09-23]` — *"extremely important … make sure it's part of the verification script."*
   Q-G3c reads it and records it in `00-inputs.md`; the config script checks every allocated PK's
   fraction against it (`chk_pk`), **the first check before any INSERT**, so a script in the wrong
   environment stops having written nothing; the verification script checks every row the run created
   against `t__CORE_INFO_S`. **Never add a typed fraction to a PK** — the BOX FE team's own
   `F___SEQUENCE(…,1) + 0.21` is right in one environment and silently wrong in the next.

   **The config script touches `BOX_FE` only** `[assumed: Edouard, 2026-09-23]` — the account that
   applies it sees nothing else. Sets mined from GBO therefore travel as literals, **read from the
   evidence CSV by `scripts/render_sql.py`, never typed**, and checked twice: the validator (SQL =
   CSV) and the verify script (BOX = GBO), which the operator runs on the read-only account.
...
10. **The INSERT is not the whole operation — call the pre-commit procedure, after classifying it.**
    ...
    **Confirmed from the body, 2026-09-24:** `p_check_Val_Curves_precommit` commits **only inside its
    loop over the configuration's step-8 rows with a NULL curve** (`023-pkg_engprecommit_body.sql:408-442`).
    With no step-8 rows it writes and commits nothing — so the rendered block calls it (the rehearsal
    too, after the self-check proves step 8 empty) and then issues **its own** `COMMIT` (rehearsal:
    `ROLLBACK`). Both pre-commits are called dynamically: an account that cannot call the package gets a
    `WARNING`, not a stop (the read-only account does not see it in Tier 2 — run 6).
...
13. **A mined FK is one of three kinds, and only one is copied.** `[confirmed: run defect, 2026-09-22]`
    ...
    | ⛔ **Re-keyed FK** — same column name, **different target table** in GBO and BOX `[2026-09-23]` | step 7 `FK_BRANCH` (GBO → branch *config*, BOX → branch); step 2 `FK_SOURCE_FRONT`/`BACK` (GBO's source systems vs BOX's) | ❌ never the mined value. The stated rule: `BRANCH_PK`; `SOURCE_FRONT`; `586.4` |
    ...
    **The test: for every FK, name the table it points into and the schema that table is in.** If the
    answer is `DEVENG`, the value is wrong. **And name the table the *GBO* column points into** — if it
    is not the same table the BOX column points into, the column is re-keyed and copying it is wrong,
    even when both tables live in a shared schema.
...
> | **Scope** | **0c** | **Blocks only the steps that depend on it** — 6, 7, 8, 11, 12, 14a per the *Blocked by* column. Steps 1–5 do not consume instrument scope and are unaffected |
>
...
| 0d | ✅ **RESOLVED 2026-09-17** — PKs come from `F___SEQUENCE(<table>,'X')`; see hard rule 3. **Per run:** callable (Q-G3b) and the target's auth code recorded (Q-G3c) | Q-G3, Q-G3b, **Q-G3c** |
...
| 2 | MIS Generic header — **`FK_SOURCE_BACK = 586.4`, `FK_SOURCE_FRONT = SOURCE_FRONT`, not mined**; curve columns = the curve's PK, **allocated here** | `T_BOX_ENGCONF_S` | `T_PGT_ENGCONF_S` | Q-02, Q-02c | 1 |
| 3 | Fixing Curve header | `T_BOX_ENGFCURVE_S` | `T_PGT_ENGFCURVE_S` | Q-03 | 2 |
| 4 | Curve → quote reference linkage — **`FK_BS` list generated from Q-04c's CSV, count-guarded; never typed** | `T_BOX_ENGLKFC_X` | `T_PGT_ENGLKFC_X` | Q-04, **Q-04c** | 3 |
...
| 7 | Accrual Exceptions — **this branch's rows only** (via branch config), approved instruments only, `FK_BRANCH = BRANCH_PK` | `T_BOX_CONFIG_ACCRUAL_S` | `T_PGT_CONFIG_ACCRUAL_S` | Q-06 | 6 |
...
| 11 | Book — batch execution registration — **`(books + 1 dummy) × instruments`** | `T_BOX_CONF_BY_BOOK_S` | **none — confirmed no GBO analogue** | Q-10, Q-10c | 0c, 5, 6 |
...
| 14a | Days Matured — **per approved instrument: insert only if missing**, `NUM_DAYS` proposed from the reference branch's environment | `T_BOX_ENGDAYS_MATURED_S` | — | Q-12 | 0c |
| 14b | EngSetup — **nothing to configure** `[stated]` | `T_BOX_ENGSETUP_S` | — | Q-12 | — |

🔄 **Reviewed by a BOX FE expert, 2026-09-23** — seven points on run 3, all reflected above and in the
step notes; **no objection to the order.** [ADR 0005](../../docs/decisions/0005-box-fe-expert-review-run-3.md).
...
is *not* FK-dependency order — **the curve's PK is allocated before the header is inserted**, because
the header points at the curve and its curve columns are `NOT NULL` (the earlier insert-then-update
fails with ORA-01400, corrected 2026-09-23) — are in [`fe-walk-notes.md`](../../docs/reference/fe-walk-notes.md).

### Notes on the steps that need them

**Moved 2026-09-22 → [`fe-walk-notes.md`](../../docs/reference/fe-walk-notes.md).** Step 1 vs 5, the
steps 2–4 allocate-first pattern, step 4b, steps 3–4's curve array, step 8's two objects, step 11's
no-GBO-analogue evidence model and its dummy-book row, step 12's Allowed Errors, step 13 and steps 14a/14b;
since 2026-09-23 also step 2's source columns and step 7's branch filter. **Read the note for the
...
- **Gate 0d** — PK mechanism, before *any* INSERT is written; and **Q-G3d**, what the called code and
  the target tables do (no `COMMIT` inside `F___SEQUENCE` or `P_ENGFixingCurve_PreCommit`, primary keys,
  triggers, every column the templates write), before the rehearsal runs.
...
2. **Authority for *that* decision.** Instrument scope, accrual values, error limits and book scope
   belong to the product-scope SME. **Technical BOX facts** — the Tier 2 dummy-book label, whether a
   parked table is needed — belong to the **BOX FE team**, and **no one can waive one of their rules**
   (run 4 "waived" the dummy row). Someone else's answer is a `PROPOSED` value awaiting theirs.
...
|---|---|
| **A named, authorised person** | Put each open decision to them **one at a time, as a decision card** (below). On an answer: record it in `00-decisions.md` and as `[stated: <name>, <role>, <date>]`, close the gate, and **emit the SQL for the steps it releases.** This is the intended path |
...
gate 0c and emits steps 6, 7, 8, 11, 12 and 14a in the same session. That is the design working, not a
shortcut.

### Decision cards — one at a time, never a list at the end `[2026-09-24]`

**Run 4 ended by listing three blockers and stopping**, with a decision-maker in the session. From now
on, the moment the run reaches something only a person can decide, it puts **one card** and waits:
*what* · *why it matters* · *steps it releases* · *evidence* · *options* · *recommendation* · *if
wrong* · *authority* · *"reply with decision, your name, your role"*. The format and the ordered list
of cards are in the run prompt. Each answer becomes a row in **`RUN_FOLDER/00-decisions.md`** (number ·
decision · value · name · role · date · evidence), which the validator reads. **A session never ends
with an unput card while `SME_IN_SESSION` names someone.**
