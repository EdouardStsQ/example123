...
| `TARGET_AUTH_CODE` | Recorded, not supplied | Read by **Q-G3c** and written into `00-inputs.md` with its PK fraction. The config script aborts if the environment it runs in has a different one |
...
3. **Never fabricate a primary key — call `F___SEQUENCE`.** `[confirmed: source, 2026-09-17]`
   ...
   **The auth code must be visible and asserted, not merely present** `[stated: BOX FE expert via
   ...
   Q-G3c reads it and records it in `00-inputs.md`; the config script re-reads it first and **aborts on
   a mismatch**, then checks every allocated PK's fraction (`chk_pk`); the verification script checks
   every row the run created. **Never type the fraction** — the BOX FE team's own `F___SEQUENCE(…,1) +
   0.21` is right in one environment and silently wrong in the next.
...
| 4 | Curve → quote reference linkage — **read in-script from `DEVENG`, count-guarded; never a typed list** | `T_BOX_ENGLKFC_X` | `T_PGT_ENGLKFC_X` | Q-04, **Q-04c** | 3, Q-04c |
