...
| `DUMMY_BOOK_LABEL` | Yes, for step 11 | The dummy-book label every instrument gets a row for. Tier 1 `26391.4`; **verified in the target by Q-10c**, never assumed |
...
| `QUOTE_REF_COLUMN` | Recorded, not supplied | Q-04c's outcome — the `T_PGT_ENGLKFC_X` column step 4's list is generated from (`FK_BS` on outcome A) |
...
| 4 | Curve → quote reference linkage — **list generated from Q-04c's CSV, count-guarded; never typed** | `T_BOX_ENGLKFC_X` | `T_PGT_ENGLKFC_X` | Q-04, **Q-04c** | 3, Q-04c |
