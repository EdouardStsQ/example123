...
1. Every FE tab (the eight SIGOM tabs) and every ACC object (portfolio properties, topic→GLTA mapping, GL accounts) has a decision-matrix row, status-tagged from the **canonical eight**: `CONFIRMED_PRESENT`, `CONFIRMED_ABSENT`, `PROPOSED`, `DERIVED`, `SME_DECISION_REQUIRED`, `EXTERNAL_CHECK_REQUIRED`, `EVIDENCE_REQUIRED`, `NOT_BRANCH_SCOPED` — the same set [`sigom-box-fe-configs-agent`](../sigom-box-fe-configs-agent/AGENT.md) uses. `propose-branch-config` feeds both agents, so a row must not change meaning depending on which one reads it.

   `DERIVED` is **not** a flavour of `PROPOSED`: it marks a value computed by a stated rule rather than read from evidence, it never counts as `CONFIRMED`, and it carries its own sign-off rather than riding along in a batch approval. Collapsing the two is precisely the failure the never-invent rule exists to prevent.
