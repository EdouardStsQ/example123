...
> scope. **Its GBO counterpart is unknown** — the naming rule would suggest `T_PGT_ERRORS_FE_S`, but
> that is a guess; check, and if none exists treat it like step 11 (BOX-only, no GBO row to mine, values
> from a reference branch's shape plus an SME).
>
