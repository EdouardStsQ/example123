...
| 3 | `03-sql/` | **Three files** `[2026-09-23]`: `<BRANCH>-<env>-config.sql` (the ordered statements, **annotated per step**, below), `-verify.sql` (read-only checks, **auth code included**) and `-rollback.sql` (the `DELETE`s, runnable — never dead code). Spec: [`fe-walk-notes.md`](../../docs/reference/fe-walk-notes.md) *The three SQL files* |
...
3. **Never fabricate a primary key — call `F___SEQUENCE`.** `[confirmed: source, 2026-09-17]`
   ...
   **The config script touches `BOX_FE` only** `[assumed: Edouard, 2026-09-23]` — the account that
   applies it sees nothing else. Sets mined from GBO therefore travel as literals, **generated from the
   evidence CSV by `scripts/evidence_to_sql.py`, never typed**, and checked twice: the validator (SQL =
