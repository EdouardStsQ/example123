...
| Run | Agent | Status |
|---|---|---|
| `NY_SCH/tier2-pre/` | `sigom-box-fe-configs-agent` | **Paused 2026-09-16** — no Tier 2 account with `BOX_FE` grants. Gate 0e is `EVIDENCE_REQUIRED — blocked on access, not on schema`; the earlier "PRE is missing tables" reading was a permissions artifact and is withdrawn. Gate 0d unresolved. Kickoff at `agents/sigom-box-fe-configs-agent/prompts/kickoff-ny-sch.md` |
| `NY_SCH/tier1-pre-rehearsal/` | `sigom-box-fe-configs-agent` | **Active, reshaped 2026-09-18** — split-tier rehearsal: mines **GBO Tier 2** (where NY_SCH is live) and writes **BOX Tier 1 PRE** (the only environment with `BOX_FE` access). Values are real; the target is a stand-in. Adds **gate 0f** (Q-14, cross-tier FK resolution). Renamed from `tier1-rehearsal/`, whose plan to mine Tier 1 was withdrawn |
