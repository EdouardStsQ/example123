...
|---|---|---|---|
| `config.sql.tmpl` | `<BRANCH>-<env>-rehearsal.sql` | **1st** — the config block ending in `ROLLBACK`; keeps nothing, run it as often as needed | `REHEARSAL OK …` |
| `config.sql.tmpl` | `<BRANCH>-<env>-config.sql` | 2nd — the same block ending in the Config pre-commit, which **COMMITs** | `DONE - committed …` |
...
1. Refuses to start if the session has an open transaction (the pre-commit's COMMIT would keep it too).
...
6. Rehearsal: `ROLLBACK`. Config: the Config pre-commit, then a check that it committed and kept the
   configuration.

Any error before the Config pre-commit stops the block and Oracle undoes everything it did.
