...
| **`auth-code-not-asserted`** / **`pk-not-checked`** / **`auth-code-mismatch`** | *2026-09-23, expert point 1.* No read of `gom_glb_sys.t__CORE_INFO_S`; an allocation without `chk_pk()`; an expected auth code that differs from `TARGET_AUTH_CODE` in `00-inputs.md` |
...
| **`quote-refs-typed`** | *Point 3.* step 4 written without reading `DEVENG.T_PGT_ENGLKFC_X` — a typed list |
| **`runtime-set-no-guard`** | a set read from `DEVENG` inside the script with no count guard — it can shrink silently |
...
`BRANCH_PK`, `SOURCE_FRONT`, `DUMMY_BOOK_LABEL`, `TARGET_AUTH_CODE`. A missing one turns its check into a
...
applies one mutation per check and asserts that exactly that check fires. **Run it after changing any
