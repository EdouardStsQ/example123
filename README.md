...
| Instrument scope not supplied | 0c | Named SME — chosen from BOX's live catalogue, not from GBO. **Now a concrete question:** a branch's set is its Accrual tab (`T_BOX_ENGACCRCONF_S`); SLB has nine. Run Q-05c and put that list to the SME |
| PK generation mechanism unknown | 0d | Q-G3 returned ten PK columns with empty `DATA_DEFAULT`, no triggers, and seven sequences unmapped to the walk tables. Points at SIGOM-side or import-tooling allocation; see the Tier 1 rehearsal |
