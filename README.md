...
| **`curve-precommit-missing`** / **`no-existing-row-guard`** | no `P_ENGFixingCurve_PreCommit`; steps 11/12/14a inserting without the abort-if-rows-exist guard the rollback relies on |
| **`step-header-incomplete`** | a step that writes rows without a *Source*, *Findings* and *If wrong* line in its header |
