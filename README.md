...
- **`skills/`** — narrow, reusable capabilities (e.g. "create legal entity in BOX", "map chart of accounts") that an agent invokes as a tool. A skill does one thing and can be tested on its own. Top-level only, never nested under an agent's folder, since more than one agent can call the same skill — see `skills/README.md`.
- **`scripts/`** — the code behind a skill built *in this repo*, one script per skill, a sibling of `skills/` rather than nested inside it (see `skills/README.md`). Empty today — every skill here is either external code (`branch-config-agent`'s, which stays in its own separate repo) or not yet implemented.
...
- **`runs/`** — the record of an actual run against a real branch/environment (queries executed, evidence CSVs, findings) — real data, unlike the placeholder-only discipline everywhere else in this repo. See `runs/README.md`.
