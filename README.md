...
> ## ▶ Next: run 6 — the SQL is rendered (2026-09-24)
>
> Run 5 ([review](03-sql/REVIEW-run-05.md)) had the right values in SQL that would not compile. From run 6
> the agent writes **`03-sql/values.json`** and `scripts/render_sql.py` writes the SQL
> ([ADR 0006](../../../docs/decisions/0006-render-sql-from-values.md)).
>
> | Before pasting the kickoff | Who |
> |---|---|
> | Archive run 5 to `archive/run-05/` (the new run overwrites the folder; `03-sql/*.sql` from run 5 would FAIL `sql-not-rendered`) | operator |
> | Fill `SME_IN_SESSION` | operator |
> | Have the BOX FE team available for card 3 (dummy label) and Q-G3d questions, if any | operator |
>
> | Applying, after the run | Expect |
> |---|---|
> | 1. `NY_SCH-tier2-pre-rehearsal.sql` (applying account) | last line **`REHEARSAL OK`**; nothing kept. Anything else: stop, send the output |
> | 2. `NY_SCH-tier2-pre-config.sql` (applying account) | last line **`DONE - committed`** |
> | 3. `NY_SCH-tier2-pre-verify.sql` (read-only account) | every query as its `Expect` line says |
> | 4. `NY_SCH-tier2-pre-rollback.sql` | only to undo 2; prints counts, then you COMMIT |
>
> ## Run 5 — after run 4's review (2026-09-24)
>
> [`REVIEW-run-04.md`](03-sql/REVIEW-run-04.md). Before pasting the kickoff: archive run 4; fill
> `SME_IN_SESSION`; ask the BOX FE team for the **Tier 2 dummy-book label** (or let the run's Q-10d
> candidates and card do it); answer run 4's question about the 124 → 116 currency-basis rows if known.
>
> ## Run 4 — after the BOX FE expert's review of run 3 (2026-09-23)
>
