...
| Blocker | Gate | Owner |
|---|---|---|
| **No Tier 2 account with `BOX_FE` grants** | 0e | Edouard / DBA. **Ask for the roles by name: `BOX_FE_RD` for the agent's mining and verification, `BOX_FE_WR` for whoever executes the script.** Those are the roles the deployment scripts themselves grant `[confirmed: 2026-09-21]`. "Tier 2 access" reads as already granted, because for GBO it is — a named role does not. Add "and the package bodies" (they are owned by `BOX_FE`). **`F___SEQUENCE` comes with the same grant set**, so this also discharges Q-G3b |
...
⛔ **The draft stays non-executable on this ground alone**, independently of gate 0e. Two actions, either
of which resolves it: read the body from the repo, or run the empirical test in Tier 1 PRE — snapshot,
INSERT, call the procedure, re-snapshot, `ROLLBACK`.
