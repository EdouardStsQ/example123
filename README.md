...
| 04-add-book-procedure.md | process | **The Book axis** — the team's own runbook for adding a new Book: Data-Lake/Control-M jobs (Part 1) then BOX FE + ACC config (Part 2). Written by the people who run it, so where it disagrees with the reverse-engineered `control-m-batch-layer.md` that disagreement is itself recorded, not resolved away |
| 05-add-product-procedure.md | process | **The Product axis** — the team's runbook for adding an instrument, across BOX API, BOX FE and BOX ACC. Marked `DOC IN PROGRESS` at source, so gaps mean unwritten. Complements `checklists/acc-add-product-checklist.md` (reverse-engineered, ACC-only) rather than replacing it |
...
| tables/t-box-cross-ref-s.md | reference | **First real per-table deep-dive.** `BOX_SYS.T_BOX_CROSS_REF_S` — the cross-system value-translation dictionary (IC → GBO): its five mapping types, confirmed columns, the new-instrument checklist, and the versioned-script convention it's maintained by |
| tables/*.md | reference | Per-table deep-dives (columns, relationships, gotchas). Further candidates live inline in `branch-config/branch-config-surface.md` §2 |
...
| job-chains/box-fe-acc-batch-runtime.md | reference | **Scheduler layer, FE + ACC.** Control-M job topology from repository configuration and DB extracts: job families and subapplications, the `T_BOX_MBJ_PROPERTIES_S` job→(branch, instrument, MBJ group) binding, and the FE→ACC handoff through Control-M events |
| job-chains/fe-batch-event-groups.md | reference | **Inside-Oracle layer, FE only.** The nine ordered FE event groups with PKs, the eight per-product Deal Data inserts, and procedure order inside groups `2629.65`/`2630.65`. Pairs with the row above — that doc has the `f_ExecuteGroup` dispatcher signature, this one has the `group` values it takes |
