...
>
> **Scope of "zero new PL/SQL", qualified 2026-09-16.** That claim is about **BOX_ACC accounting
> logic** and it still stands there. It does **not** extend to the rest of adding a product: the team's
> own runbook ([05-add-product-procedure](../05-add-product-procedure.md)) requires substantial
> per-product PL/SQL in **`BOX_FE`** — a new `PKG_ENGGN` constant, a `PKG_FE_DATADEAL_LOAD` import
> process, `elsif` branches in `PKG_ENGMAIN_OPTZ`, two new packages per product
> (`Pkg_engmain<product>_optz`, `Pkg_eng<product>_optz`) with role grants, and edits to
> `PKG_FE_MTM_CALCULATION` — plus a change to
> `BOX_TRD.PKG_BOX_DEAL_LITE_DATA_API.f_CalculateInstrumentType` on the API side. Read "config-only" as
> scoped to the accounting engine, not to product onboarding end to end.
...
> **Also required for a new instrument, and not covered below: cross-reference mappings.**
> `BOX_SYS.T_BOX_CROSS_REF_S` needs **five rows minimum** per new instrument — one `Instrument`
> mapping, two `Direction` and two `Direction - Leg` — before its IC-domain values translate into GBO
> Static-Data codes. That is a `BOX_SYS` concern rather than a `BOX_ACC` one, so it sits outside this
> checklist's scope, but it is genuinely part of "add a product" and is easy to miss. Procedure,
> worked example and the versioned-script convention:
> [t-box-cross-ref-s](../../reference/tables/t-box-cross-ref-s.md).
>
> Note it independently corroborates this page's own §3 row "Sigom instrument label | `SWAP`": the
> cross-reference maps IC `IRS` → GBO `SWAP`.
