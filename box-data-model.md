...
- **Folder** = an **aggregation of trades / Murex portfolios**, owned by the external system **GER** and
  distributed to consuming systems including BOX. Mastered here in `T_PGT_FOLDER_S`.
- **Book** (a.k.a. Libro, Label) = **the desk**. A book contains many folders. Mastered in
  `PGT_SYS.PGT_DOMAINS` where `FK_OWNER_OBJ = 17910.4`, and registered for batch execution in
  `T_BOX_CONF_BY_BOOK_S` (FE) and `T_BOX_MBJ_PROPERTIES_S` (ACC).
...
**➡️ Full treatment in [`book-and-folder.md`](book-and-folder.md)** — the containment model, why
processing partitions on (Book, Instrument, Branch), the missing Book↔Folder mapping, and the
deal-level query that derives it anyway.

✅ **BOX-side folder master — resolved 2026-09-21.** `[confirmed: DB]` This page previously flagged as
open whether `BOX_ACC.T_BOX_NETCONTRACT_S.FK_FOLDER` pointed at `PGT_STC.T_PGT_FOLDER_S` or an unnamed
BOX twin. The metamodel declares it: `BOX - Net Contract.pFolder → PGT - Folders →
PGT_STC.T_PGT_FOLDER_S`, and the same for Acct Movements, Acct Key and Grouped Movements. **There is no
BOX folder twin.** A declaration, not a name search.

**And SIGOM treats Folder and portfolio as the same thing** — `BOX - Migration Criteria` and
`BOX - Auki Criteria` both declare a field named **`apPortfolio`** targeting `PGT - Folders`. The
"Folder ≈ Murex portfolio context" reading above is now confirmed from the system's own vocabulary.
...
| FK_OWNER_OBJ | `GOM_GLB_SYS.T__OBJ_DEF_S` | **The SIGOM object (screen) the row is edited through** — not row data, and not one value per module. `35000126.65` = `BOX_ENG_Config`, `35000123.65` = `BOX_ENG_FixingCurve`, `12198.4` = the GBO FE object `[confirmed: DB, 2026-09-18]`. Never copied across a GBO→BOX INSERT; see [`sigom-metamodel.md`](sigom-metamodel.md) §3 |
...
| FK_EXTENSION | `GOM_GLB_SYS.T__EXT_DEF_S` | **Which field/tab of that screen** the row belongs to. NULL on an object's base table; set on its extensions — `35001114.65` Accrual, `35001566.65` branch bridge, `35001101.65` quote array. Derived from the metamodel (Q-G6/Q-G7), never mined |
