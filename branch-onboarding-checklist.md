...
| 1.3 | Review **Books**; for new Books generate config + **Control-M** to load BU (deal/flow/market) | **Procedure: [04-add-book-procedure](../04-add-book-procedure.md)** (the team's own runbook, both Data-Lake and BOX FE/ACC halves). Topology: [box-branch-batch-controlm](../../reference/job-chains/control-m-batch-layer.md) — job count **contested**: 6/book per the repo reading, 4/book per the runbook | `confirmed` (that books need jobs) / `open-question` (how many, and which job family) | 🟢 automate |
...
| 1.6 | **Folders** — does the new branch need its own Folder rows? | `PGT_STC.T_PGT_FOLDER_S`, branch-keyed via `FK_BRANCH`; SIGOM `GBO > Static Data > Environment > Config > Folders`. Query + columns in [box-data-model](../../reference/box-data-model.md#folders--branch-keyed-gbo-static-data) | `open-question` — see note | 🔴 check |
...
> **Note on 1.6 — Folders is a newly-surfaced candidate item, not a confirmed requirement**
> `[confirmed: DB via BOX FE Developer, 2026-09-15]` for the table; `[open-question]` for the step.
>
> `PGT_STC.T_PGT_FOLDER_S` is **branch-keyed**, which is what puts it on this checklist at all — most
> static data isn't. Folder is the portfolio-side dimension (distinct from Book, the Data-Lake
> processing dimension — see [box-data-model](../../reference/box-data-model.md#folders--branch-keyed-gbo-static-data)),
> and it travels with a deal into `T_BOX_DATADEAL_S` and `BOX_ACC.T_BOX_NETCONTRACT_S.FK_FOLDER`.
>
> What is **not** established: whether onboarding a branch requires creating Folder rows for it, who
> owns that (GBO-side, like branch creation itself?), and whether BOX reads this same `PGT_STC` table
> or has an unnamed twin. Until answered, this row is a **check**, not work to plan — and it is
> deliberately numbered 1.6 rather than inserted mid-sequence so the existing item numbers other
> documents cite stay stable. Resolve it by asking the BOX Lead the same way branch-group membership
> was resolved, rather than by inferring from Madrid's folder list.
...
| 3.6 | **Control-M Accounting Batch** by instrument & book | `T_BOX_BRPROCCAL_S` / `_QUEUE_S_OPTZ` + `T_BOX_MBJ_PROPERTIES_S`; topology in [box-fe-acc-batch-runtime](../../reference/job-chains/box-fe-acc-batch-runtime.md) §3 (ACC MBJ groups `3275.65` General Accounting / `3375.65` MTM-Revaluation) | `confirmed` | 🟢 automate |

## Layer 4 — BOX FE

> **One qualification, 2026-09-16.** `[stated: BOX Developer via Edouard]` The items below are still
> sourced by mining their GBO equivalents — that is unchanged. The single exception is **instrument
> scope**: which instruments a branch gets is chosen from **what is already created and live in BOX**,
> by a named SME, not read from the branch's GBO instrument configuration. See
...
| 4.4 | Create **Control-M Financial Batch** by instrument & book (FE side) | `T_BOX_BRPROCCAL_S`; Control-M topology in [box-fe-acc-batch-runtime](../../reference/job-chains/box-fe-acc-batch-runtime.md) §2, the event groups those jobs dispatch into in [fe-batch-event-groups](../../reference/job-chains/fe-batch-event-groups.md) | `confirmed` | automate |
