...
**Extended and partly reframed, 2026-09-16** — full deep-dive in
[`../tables/t-box-cross-ref-s.md`](../tables/t-box-cross-ref-s.md).

Every schema-level claim above holds: the column list is confirmed from a real DML script
(`PK`, `FK_OWNER_OBJ`, `DESCRIPTION`, `MAP_TYPE`, `MAP_FROM`, `MAP_TO`, `MAP_TO_TYPE`,
`FK_INSTRUMENT_FILTER`), and there is indeed no branch or entity column. Two things are now different:

**There are five map types, not three.** `Instrument`, `Direction`, `Direction - Leg`, `Flow Type` and
`Source` — the paragraph above named source, instrument and direction, missing the leg-direction and
flow-type variants. `Direction` and `Direction - Leg` draw from different source vocabularies
(`RECEIVE`/`PAY` vs `SELL`/`BUY`) onto the same GBO targets (`BOR`/`LOA`), and both require the
instrument filter that the other three leave null.

**"Supports online value translation" may be the wrong frame.** The team's own page describes this
table's purpose as translating values *from the **Integrity Check (IC)** domain into GBO Static-Data
equivalents* — every `MAP_FROM` is drawn from an IC catalogue (Financial Instruments, Flow Mappings,
Leg Mappings, Source Systems). That is a reconciliation/validation vocabulary, not obviously the
real-time arrival path this repo has associated with "CROSS_REF" throughout. One dictionary can serve
several consumers, so this does not disprove an online consumer — and the sentence above was careful
to say the consumer is an unproven external dependency. But **do not treat "CROSS_REF" and "online
arrival" as synonyms**; the only documented consumer is Integrity Check. `[open-question]` — find what
actually reads this table before concluding either way.
