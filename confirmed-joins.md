...
| `PGT_SYS` | System config — Family/Product/Sub-Product, `PGT_DOMAINS` | ✅ **Yes** for the product hierarchy — ⚠️ **not for every `PGT_DOMAINS` row**: Tier 1's dummy book label `26391.4` is absent in Tier 2 `[confirmed: DB, run 4, 2026-09-24]`. Check a label in the target before using it |
...
> ✅ **Challenged 2026-09-23, settled 2026-09-24 — `FK_BS`.** A BOX FE expert's query selected `PK`;
> the operator (BOX Dev) confirmed the link is `T_PGT_ENGLKFC_X.FK_BS = T_PGT_QUOTE_REFERENCE_S.PK`,
> as recorded. The side-by-side test run 4 used was itself wrong: a bridge PK coinciding with a quote
> reference PK (`87.35`) is a number shared across tables, not a relationship.
...
## Re-keyed columns — same name, different target table `[stated: BOX FE expert via Edouard, 2026-09-23]`

**A column with the same name in GBO and BOX is not the same FK.** Copying one of these is a silent
defect: the value inserts, resolves to *something*, and is wrong.

| Column | GBO → | BOX → | Write |
|---|---|---|---|
| `T_*_CONFIG_ACCRUAL_S.FK_BRANCH` | `PGT_STC.T_PGT_BRANCH_CONFIG_S` (a branch **config**; NY `141.35`) | `PGT_STC.T_PGT_BRANCH_S` (the branch; NY `20007.4`) | `BRANCH_PK` |
| `T_*_ENGCONF_S.FK_SOURCE_BACK` | GBO's back-office source system | BOX's — **`586.4`, always** | `586.4` |
| `T_*_ENGCONF_S.FK_SOURCE_FRONT` | GBO's front source system | the **Murex instance** the branch's trades come from (NY: Murex 3 Latam `513.4`) | input `SOURCE_FRONT` |

The GBO branch-config chain, from the expert's query: `T_PGT_CONFIG_ACCRUAL_S.FK_BRANCH →
T_PGT_BRANCH_CONFIG_S.PK`, then `T_PGT_BRANCH_CONFIG_S.FK_BRANCH → T_PGT_BRANCH_S.PK`. BOX's
`FK_BRANCH` on `T_BOX_CONFIG_ACCRUAL_S` and `T_BOX_ERRORS_FE_S` holds the branch PK directly — the BOX FE
team's own inserts write `22.21` (Madrid) and `20087.4` (SLB). **Before copying any FK, name the table
the GBO column points into as well as the BOX one** (charter hard rule 13).
...
| `T_BOX_ERRORS_FE_S` (Allowed Errors) — ⚠️ **the twin exists but is no longer mined for walk step 12; limits are proposed from a reference branch, [ADR 0004](../decisions/0004-step12-limits-from-reference-branch.md)** | `DEVENG.T_PGT_ERRORS_FE_S`, keyed by **`FK_BRANCH`** `[confirmed: DB, 2026-09-18]` |
