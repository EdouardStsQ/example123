...
> ⚠️ **Challenged 2026-09-23.** A BOX FE expert selects the quote references as
> `SELECT DISTINCT PK FROM DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = 1.35` — **`PK`, not `FK_BS`.** The
> row above stays until **Q-04c** runs both side by side; whichever outcome it gives is recorded here.
