...
## SIGOM identity columns — `FK_OWNER_OBJ` and `FK_EXTENSION` belong to the *destination* table

`[confirmed: DB via Edouard, 2026-09-18]` **These are not mineable values. They are constants of the
table you are writing into, and they must be read from that table — never carried across from the GBO
row.**

The evidence is a clean pair:

| Table | `FK_OWNER_OBJ` observed | Environment of that PK |
|---|---|---|
| `DEVENG.T_PGT_ENGCONF_S` (GBO, Tier 1) | `12198.4` — **the same on every row** | `.4` global |
| `BOX_FE.T_BOX_ENGCONF_S` (BOX, Tier 1) | `35000126.65` — **the same on every row** | `.65` BOX-DEV |
| `BOX_FE.T_BOX_ENGACCRCONF_S` (BOX, Tier 1) | `35000126.65`, `FK_EXTENSION 35001114.65` | `.65` BOX-DEV |
| `BOX_FE.T_BOX_ENGCONF_X` (BOX, Tier 1) | `35000126.65`, `FK_EXTENSION 35001566.65` | `.65` BOX-DEV |

Read across those rows:

- **`FK_OWNER_OBJ` identifies the owning SIGOM module**, not the branch or the configuration.
  `35000126.65` is *the BOX Financial Engine module*; `12198.4` is *the GBO Financial Engine module*.
  Every BOX FE walk table seen so far carries the same one.
- **`FK_EXTENSION` identifies the table/screen within that module** — `35001114.65` for Accrual,
  `35001566.65` for the branch-association bridge. It varies per table, not per row.
- Both are `.65`, meaning they were **allocated in BOX-DEV** and promoted as metadata. That is
  consistent with them being platform objects rather than branch data.

**The rule.** For every INSERT in the walk, `FK_OWNER_OBJ` and `FK_EXTENSION` come from
`SELECT DISTINCT FK_OWNER_OBJ, FK_EXTENSION FROM BOX_FE.<target table>` in the **target environment**
(Q-G6), and from nowhere else. A `SELECT … FROM DEVENG.T_PGT_*` that carries these columns into a
`BOX_FE` INSERT writes the GBO module's identity into a BOX row — structurally wrong, and it will
either violate a constraint or, worse, create a row that no SIGOM screen can see.

⚠️ **Whether `35000126.65` is the same value in Tier 2** is unconfirmed. It is a `.65` PK, so it was
allocated once in BOX-DEV and presumably promoted everywhere, but "presumably" is not this repo's
standard. Q-G6 answers it in one read, and it is part of the deferred set until `BOX_FE` Tier 2 access
lands. **Do not hard-code `35000126.65` into a script.**

`[open-question]` What object `35000126.65` actually *is* — which catalogue table holds it — has not
been looked up. The onboarding does not need it, but it would make the model legible rather than
empirical.
