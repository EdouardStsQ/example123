...
| `pnl_type_vr` · `status_vr`/`status_native_vr` · `sens_description` | EOD · `BOValidated` · "P&L LEG+P&L FX" | boilerplate |
...
3. For each book, **generate the 6 jobs** (DD base/OTC/SQL/OTCSQL + FD + MD) from the template,
   substituting `country/source/book`, the `<BOOK-ABBREV>`, and dataset names.
...
**Automatable?** 🟢 High — everything except the exact upstream event names + the book→abbrev mapping
is boilerplate. Those two are learnable from the analogous branch's folder; genuinely-new feeds are
