...
⚠️ **Those seven columns are the whole of what crosses from GBO to BOX on this table** — and since
2026-09-23 only three are copied: `FK_SOURCE_BACK` is always `586.4` and `FK_SOURCE_FRONT` is the branch's
Murex instance, both BOX's own values; the curve columns are this run's curve (Q-02, ADR 0005).
`FK_OWNER_OBJ` and `FK_EXTENSION` are **constants of the BOX table itself** — `35000126.65` on every
`BOX_FE.T_BOX_ENGCONF_S` row, against `12198.4` on every `DEVENG.T_PGT_ENGCONF_S` row (the GBO FE
module). `[confirmed: DB via Edouard, 2026-09-18]` Read them from the destination with **Q-G6**; copying
them from the GBO row produces a BOX row owned by the GBO module. Charter hard rule 9, and the defect
that sank the first NY_SCH draft. On the bridge itself the Tier 1 pair is
`FK_OWNER_OBJ = 35000126.65`, `FK_EXTENSION = 35001566.65`.
...
| **Allowed Errors** | `T_BOX_ERRORS_FE_S` | `T_PGT_ERRORS_FE_S` (keyed by `FK_BRANCH`) — ⚠️ **exists, but not mined for the walk since 2026-09-21: limits are proposed from a reference branch, [ADR 0004](../../decisions/0004-step12-limits-from-reference-branch.md)** | error limit before a process crashes, **per instrument × per branch**. SIGOM: `BOX - Financial Engine > Process Management > Allowed Errors` | `confirmed` — table, grain and GBO twin `[stated: BOX FE Developer, 2026-09-17; GBO twin confirmed: DB, 2026-09-18]` |
...
> Return Swap**, whose `FK_INSTRUMENT` values **did not match** in that join. The 7 in the table
> above was very likely measured the same way.
>
> ### ⚠️ Correction, 2026-09-22 — "did not match" is not "has no row"
>
> `[confirmed: DB via Edouard, 2026-09-22]` **Credit Derivatives IS in `PGT_SYS.T_PGT_SUB_PRODUCT_S`** —
> PK `20313.4`, CODE `Credit`, `FK_PARENT 20326.4`. The table holds 50 rows, and ordered by
> `DESCRIPTION` the first is *Caps And Floors*, so **no Sub-Product description begins with A or B** —
> Bond Return Swap is not there under that name.
>
> The original text above was read as *"these instruments have no Sub-Product row"* and propagated
> across the repo tagged `[confirmed]`. **That was an inference, not a reading.** An inner join
> returning fewer rows is consistent with the row being absent, the join being wrong, **or the FK
> holding a different value** — which is hard rule 8 applied to this repo's own conclusion.
>
> **The open question is now narrower and cheaper:** what does SLB's Credit Derivatives accrual row
> actually hold in `FK_INSTRUMENT`, and how does it differ from `20313.4`?
>
