...
| **Allowed Errors** | `T_BOX_ERRORS_FE_S` | `[open-question]` | error limit before a process crashes, **per instrument × per branch**. SIGOM: `BOX - Financial Engine > Process Management > Allowed Errors` | `confirmed` (table + grain) `[stated: BOX FE Developer, 2026-09-17]` |
...
>
> A direct read of **SLB's Accrual tab in Tier 1** shows **nine** rows, not the seven in the table
> above: OTC Option, Cross Currency Swap, Swap, Cash Flow Matching, Deposit & Loan, Credit Derivatives,
> Forward Rate Agreement, Caps And Floors, Bond Return Swap. The 7 is older and its provenance isn't
> recorded; both are kept until someone re-runs the count, with the 2026-09-17 direct observation
> treated as the better evidence. Neither is a target for a new branch — the point of this table is
> that two live branches legitimately differ.
