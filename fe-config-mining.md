...
### Q-G6 — SIGOM identity constants for the target tables `Env: BOX` *(new 2026-09-18)*

**Purpose:** read `FK_OWNER_OBJ` and `FK_EXTENSION` from the **destination** table. These are constants
of the table being written, never values mined from GBO — charter hard rule 9.
...
**Tier 1 observed values** `[confirmed: DB via Edouard, 2026-09-18]`:

| Table | `FK_OWNER_OBJ` | `FK_EXTENSION` |
|---|---|---|
| `T_BOX_ENGCONF_S` | `35000126.65` | — |
| `T_BOX_ENGCONF_X` | `35000126.65` | `35001566.65` |
| `T_BOX_ENGACCRCONF_S` | `35000126.65` | `35001114.65` |

For contrast, the GBO side: `DEVENG.T_PGT_ENGCONF_S` carries `FK_OWNER_OBJ = 12198.4` on **every** row.
That is the GBO Financial Engine module. Copying it into a BOX row is the 2026-09-18 draft defect.

⚠️ **These are Tier 1 values. Do not hard-code them.** Bind them from this query's result in the target
environment. In a deferred-verification run Q-G6 is part of the deferred set, so the draft carries them
as named substitution variables (`&&OWNER_OBJ`, `&&EXT_<table>`) and they resolve at release.
...
> `EVIDENCE_REQUIRED` with the product-shaped-branch question attached, and let the SME sign off on a
> concrete row set. A blocker with no rows behind it gives the reviewer nothing to disagree with.
>
