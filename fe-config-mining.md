...
**For BOX ACC**, swap the module to `35000006.65` — 31 objects, 269 fields, read 2026-09-21 and recorded
in [`../sigom-metamodel.md`](../sigom-metamodel.md) §6b. ACC is spread across five more modules
(`35000007.65` settlement, `30000000.65` deal data, `30000003.65` platform, `774.66` account/SSI,
`35000009.65` AUKI migration); to find a module without guessing, enumerate rather than assume:

```sql
SELECT FK_PARENT AS module_pk, COUNT(*) AS objects, MIN(PK_NAME), MAX(PK_NAME)
FROM   GOM_GLB_SYS.T__OBJ_DEF_S GROUP BY FK_PARENT ORDER BY objects DESC;
```
...
> configuration.** BOX FE and BOX ACC both run processing by **(BOOK, INSTRUMENT, BRANCH)** to
> parallelise jobs. A row here declares one unit of parallel work. Its ACC twin,
> `T_BOX_MBJ_PROPERTIES_S`, carries the same triple *plus* `Workers`, `Task_per_worker` and
> `Concurrence` — which is the proof.
>
