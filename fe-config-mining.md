...
1. **Don't type the list.** Agreed without reservation — fixed structurally: step 4's list is
   **generated** from this query's CSV by `scripts/evidence_to_sql.py`, the script guards its length,
   the validator checks it equals the CSV, and the verification script compares BOX with GBO both ways.
   (Reading `DEVENG` inside the script is not an option: the applying account sees `BOX_FE` only.) A 38-value *typed* list is transcription, and transcription is
...
**The step-4 SQL, once the outcome is A** — generated, count-guarded, never typed:

```bash
python3 scripts/evidence_to_sql.py list runs/NY_SCH/tier2-pre/01-evidence/Q-04c-quote-ref-column-gbo.csv FK_BS
# → sys.odcinumberlist(6400.4, …)      and on stderr: -- 38 distinct FK_BS
```

```sql
v_quote_refs sys.odcinumberlist := <the generated literal>;          -- DECLARE section
…
IF v_quote_refs.COUNT <> c_expected_quote_refs THEN
  RAISE_APPLICATION_ERROR(-20004, 'Step 4: list has '||v_quote_refs.COUNT||', evidence says '||c_expected_quote_refs);
END IF;
FOR i IN 1 .. v_quote_refs.COUNT LOOP
  ...
  VALUES (v_pk, c_owner_fcurve, c_ext_englkfc, v_curve_pk, v_quote_refs(i));   -- only FK_BS is mined
END LOOP;
```

The GBO curve PK (`1.35`) never appears in the config script: it is the filter behind the evidence and
lives in the verify script's V4. Record the outcome as `QUOTE_REF_COLUMN` in `00-inputs.md` — the
validator reads it to know which CSV column the list must equal.
...
**All branches, unfiltered** — the filter is applied by the generator (`--where BRANCH_PK=…`) and again
by the validator, visibly, so the findings can list what was dropped. `LEFT JOIN`s: a row whose branch
...
| **2026-09-23** | Q-04c | **New, blocks step 4** — which `_X` column is the quote reference, `PK` or `FK_BS`; and the CSV step 4's list is generated from |
