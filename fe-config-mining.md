...
```sql
-- Run in BOTH environments, compare. This is the only per-pair check that earns its keep.
SELECT qr.PK, qr.DESCRIPTION, qs.DESCRIPTION AS QUOTE_SOURCE
...
**Expect:** every quote reference behind the branch's curve resolving identically in both. If any is
absent in the target, step 4 is genuinely blocked there and that **is** a real finding — unlike a
`PGT_STC`/`PGT_SYS` value, and unlike a source-object PK.
