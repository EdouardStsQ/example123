...
`<OWNER>.PKG_ENGPRECOMMIT`, which works whether or not a synonym exists.

> ## ✅ (d) returning 0 rows is not a stop `[confirmed: DB via operator, Tier 2 PRE, 2026-09-24]`
>
> Run 6 got **0 rows** for `PKG_ENGPRECOMMIT` on the operator's read-only account and stopped. `ALL_OBJECTS`
> lists only objects **that account** may use: 0 rows means *not granted to it*, not *absent*. The package
> is in the `BOX_FE` source (`cib-boxfin-dbboxfe`, `07-BOX_FE/…/023-pkg_engprecommit_body.sql`), and it
> is the **applying** account that calls it. So: record the owner as **`BOX_FE`**, source
> `[confirmed: source, cib-boxfin-dbboxfe 023-pkg_engprecommit_body.sql] — Q-G3b (d) not visible to the
> read-only account`, and continue. The rendered block calls both pre-commits **dynamically**: if the
> applying account cannot call them, the rehearsal prints a `WARNING` and carries on, so the answer
> arrives in the rehearsal without stopping the run. Only an `F___SEQUENCE` that the applying account
> cannot call is a stop (every PK depends on it) — and the rehearsal shows that too, writing nothing.
...
> ## ✅ (a) and (b) answered from the source, 2026-09-24 — run only (c) and (e)
>
> `[confirmed: source, cib-boxfin-dbboxfe, 07-BOX_FE, development branch, via operator, 2026-09-24]`
>
> | Routine | COMMIT / ROLLBACK / autonomous / WHEN OTHERS | DML | On failure |
> |---|---|---|---|
> | `F___SEQUENCE(t,'X')` (`000-f___sequence.sql:4-28`) | none | none — reads `SQ_BOX_FINANENG1` (`SQ_BOX_FINANENG3` for `T_BOX_ENGPROCESS_S`/`T_BOX_ENGDOM_S`) and `gom_glb_sys.t__CORE_INFO_S`, caching the fraction in `pkg_engcore.fractional` | — |
> | `P_ENGFixingCurve_PreCommit(curve)` (`023-…body.sql:275-363`) | none | none — reads `T_BOX_ENGFCURVE_S`, `T_BOX_ENGFIXDISC_S`, `T_BOX_ENGLKFC_X`, `pgt_*` | **raises** `-20001` when a curve it needs is missing |
> | `p_check_Val_Curves_precommit(conf)` (`023-…body.sql:395-443`) | **COMMIT only inside its loop over the configuration's `T_BOX_FIXING_BY_INSTR_S` rows with a NULL curve** (`:408-442`); no ROLLBACK, no exception section | `UPDATE T_BOX_FIXING_BY_INSTR_S` only | an unhandled `NO_DATA_FOUND` if the header is missing |
>
> **Consequence:** with no step-8 rows the Config pre-commit writes nothing and **commits nothing**. So
> the config block ends with its **own `COMMIT`**, and the rehearsal can run the pre-commit too (the
> self-check proves step 8 has no rows first). **No triggers** on the ten tables in the source
> (caveat: development branch; `ALL_TRIGGERS` in the target is definitive — run (b) if it is visible).
...
template defect, never work round it. (a) and (b): see the box above — answered from the source. (The
block still checks at run time that its transaction was never ended midway, error -20098, before the
final COMMIT or ROLLBACK.) The block calls `DBMS_TRANSACTION`, granted to `PUBLIC` by default; if a
hardened database revoked it, the rehearsal fails to compile (PLS-00201) and writes nothing — ask the DBA.

→ `Q-G3d-triggers.csv` (b, if visible), `Q-G3d-keys.csv` (c), **`Q-G3d-columns.csv`** (e)

### Q-G3e — Is the sequence ahead of every PK already in the tables? `Env: BOX` (target tier) *(new 2026-09-24)*

**Why:** `[stated: BOX Dev via operator, 2026-09-24]` *"Tier 2 may be using fixed PKs in some cases.
Everyone should use `F___SEQUENCE`; the Tier 1 procedure is the correct one, but be extra careful in
Tier 2."* A row inserted with a typed PK that carries this environment's fraction can sit **ahead of the
sequence**; the next `F___SEQUENCE` value would then collide with it. The rendered block already guards
this at run time — `chk_pk` checks every new PK is unused and stops (-20003, nothing kept) — so this
query tells you **before** the rehearsal whether that guard will fire.

```sql
-- (a) the sequence F___SEQUENCE uses for every walk table (may return 0 rows on a read-only account)
SELECT SEQUENCE_OWNER, SEQUENCE_NAME, LAST_NUMBER, INCREMENT_BY, CACHE_SIZE
FROM   ALL_SEQUENCES WHERE SEQUENCE_NAME = 'SQ_BOX_FINANENG1';

-- (b) per table: the highest PK integer carrying this environment's fraction (&&FRACTION from Q-G3c,
--     e.g. 0.44), and how many rows carry it vs. any other fraction
SELECT 'T_BOX_ENGCONF_S' AS t, MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END) AS max_int_env,
       COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END) AS n_env, COUNT(*) AS n_all FROM BOX_FE.T_BOX_ENGCONF_S
UNION ALL SELECT 'T_BOX_ENGFCURVE_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ENGFCURVE_S
UNION ALL SELECT 'T_BOX_ENGLKFC_X', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ENGLKFC_X
UNION ALL SELECT 'T_BOX_ENGCONF_X', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ENGCONF_X
UNION ALL SELECT 'T_BOX_ENGACCRCONF_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ENGACCRCONF_S
UNION ALL SELECT 'T_BOX_CONFIG_ACCRUAL_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_CONFIG_ACCRUAL_S
UNION ALL SELECT 'T_BOX_CONF_BY_BOOK_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_CONF_BY_BOOK_S
UNION ALL SELECT 'T_BOX_ERRORS_FE_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ERRORS_FE_S
UNION ALL SELECT 'T_BOX_ENGDAYS_MATURED_S', MAX(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN TRUNC(PK) END), COUNT(CASE WHEN PK - TRUNC(PK) = &&FRACTION THEN 1 END), COUNT(*) FROM BOX_FE.T_BOX_ENGDAYS_MATURED_S;
```

**Read it:** if (a) is visible and any `max_int_env ≥ LAST_NUMBER − CACHE_SIZE × INCREMENT_BY`, a typed
PK sits where the sequence will go — **a question to the BOX FE team before the rehearsal**, never a
reason to type PKs ourselves. If (a) is not visible, (b) is still the evidence: the rehearsal's `chk_pk`
is then the check, and it stops with nothing kept. Rows with **other** fractions cannot collide (the
fraction is part of the PK). Record the result in the findings under gate 0d.

→ `Q-G3e-sequence.csv` (a), `Q-G3e-pk-headroom.csv` (b)
