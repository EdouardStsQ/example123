...
| Where | What it does |
|---|---|
| **Config script, first block** | Re-reads `auth_code` at execution time, prints it, and **aborts** (`RAISE_APPLICATION_ERROR`) if it differs from `TARGET_AUTH_CODE` — a script run in the wrong environment stops before its first INSERT |
| **Config script, every allocation** | `chk_pk(v_pk)` after each `F___SEQUENCE` call: the PK's fractional part must equal `TARGET_PK_FRACTION`, or abort |
...
### Q-04c — Which column holds the quote reference? and the source set for step 4 `Env: GBO` + `BOX` *(new 2026-09-23)* ⛔ **blocks step 4**

**Purpose:** settle a disagreement before run 4 writes step 4 again. `[stated: BOX FE expert via
Edouard, 2026-09-23]` *"I don't like that we list the actual values in the sql file, how are we sure
they are correct? For NY these should be picked from query `SELECT DISTINCT PK FROM
DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = 1.35`. I did a quick check and I'm not sure the values your
sql file inserted were correct."*

**Two separate points, and they have different fixes:**

1. **Don't type the list.** Agreed without reservation — fixed structurally: step 4 now reads its set
   from `DEVENG` inside the script (a cursor, below), guards the count, and the verification script
   compares the two sets both ways. A 38-value literal list is transcription, and transcription is
   where run 3 could have gone wrong.
2. **Which column is the quote reference — `PK` or `FK_BS`?** The expert's query copies **`PK`**. This
   catalogue's Q-04 copies **`FK_BS`**, from a join `[confirmed: DB via BOX Lead, 2026-09-10]` —
   `t1.FK_BS = T_PGT_QUOTE_REFERENCE_S.PK` — and the `_X` convention (`FK_PARENT` up, `FK_BS` across)
   confirmed on three tables. **Both cannot be right.** If the element is `PK`, then `_X` tables do
   not take a `F___SEQUENCE` PK at all, and hard rule 3 needs an exception. **Not decided by argument —
   by this query:**

```sql
-- (a) GBO: for NY's curve, which column resolves to a quote reference?
SELECT t1.*,
       (SELECT COUNT(*) FROM PGT_MRK.T_PGT_QUOTE_REFERENCE_S q WHERE q.PK = t1.PK)    AS pk_is_quote_ref,
       (SELECT COUNT(*) FROM PGT_MRK.T_PGT_QUOTE_REFERENCE_S q WHERE q.PK = t1.FK_BS) AS fk_bs_is_quote_ref
FROM   DEVENG.T_PGT_ENGLKFC_X t1
WHERE  t1.FK_PARENT = &&GBO_CURVE_PK                -- NY: 1.35, from Q-03. The CURVE.
ORDER  BY t1.PK;

-- (b) Counts: rows, distinct PK, distinct FK_BS. Differences mean duplicates — say which column repeats.
SELECT COUNT(*) AS n_rows, COUNT(DISTINCT PK) AS n_pk, COUNT(DISTINCT FK_BS) AS n_fk_bs
FROM   DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = &&GBO_CURVE_PK;

-- (c) BOX: the same shape on a curve BOX already has — does BOX's own _X row carry an allocated PK?
--     Use any existing BOX curve (Q-03b); in Tier 1, SLB's.
SELECT t1.*,
       (SELECT COUNT(*) FROM PGT_MRK.T_PGT_QUOTE_REFERENCE_S q WHERE q.PK = t1.PK)    AS pk_is_quote_ref,
       (SELECT COUNT(*) FROM PGT_MRK.T_PGT_QUOTE_REFERENCE_S q WHERE q.PK = t1.FK_BS) AS fk_bs_is_quote_ref
FROM   BOX_FE.T_BOX_ENGLKFC_X t1
WHERE  t1.FK_PARENT = &&EXISTING_BOX_CURVE_PK;
```

**Read the result — three outcomes:**

| Outcome | Meaning | Step 4 does |
|---|---|---|
| **A** — `fk_bs_is_quote_ref = 1` on every row, `pk_is_quote_ref = 0`, and (c) shows the same | Q-04 is right; the expert's query returns bridge-row PKs | Copies **`FK_BS`**; `PK` from `F___SEQUENCE` — as now. **Tell the expert, with the CSV** |
| **B** — `pk_is_quote_ref = 1` on every row, and (c) shows the same | The expert is right; `PK` **is** the element and is not allocated on `_X` tables | ⛔ **Hold steps 2–5** (`EVIDENCE_REQUIRED`) and raise it: hard rule 3 and the `_X` convention both need an ADR before anything is written; a curve without its array is incomplete, and step 5's `T_BOX_ENGCONF_X` is an `_X` table too |
| **C** — mixed, or GBO and BOX differ | The two schemas key the bridge differently | ⛔ Hold steps 2–5; source question to the BOX FE team |

**Either way, compare run 3's 38 values** (archived `03-sql/`) against both columns of (a) and say which
column they came from — that answers the expert's *"I'm not sure the values were correct"* directly.

**The step-4 SQL, once the outcome is A** — read in-script, count-guarded, never typed:

```sql
-- expected count from Q-04c (b): n_fk_bs
SELECT COUNT(DISTINCT FK_BS) INTO v_n FROM DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = c_gbo_curve_pk;
IF v_n <> c_expected_quote_refs THEN
  RAISE_APPLICATION_ERROR(-20004, 'Step 4: '||v_n||' quote refs in DEVENG, expected '||c_expected_quote_refs);
END IF;
FOR r IN (SELECT DISTINCT FK_BS FROM DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = c_gbo_curve_pk) LOOP
  ...
  VALUES (v_pk, c_owner_fcurve, c_ext_englkfc, v_curve_pk, r.FK_BS);    -- only FK_BS is mined
END LOOP;
```

`c_gbo_curve_pk` is a **source filter**, never written to a BOX column (hard rule 13). Only one column
crosses from `DEVENG`; the other four are allocated, structural or intra-config — so this is **not**
the forbidden `INSERT … SELECT FROM DEVENG`, which carries whole rows. **It requires the executing
account to read `DEVENG`** — if it cannot, the block fails to compile and nothing is inserted, which is
the safe failure. Whether that grant exists is a question for the BOX FE team (charter *Escalation*).

→ `Q-04c-quote-ref-column.csv` (three files: `-gbo`, `-counts`, `-box`)
...
branch in the target tier. **(a) empty or (c) zero in the target is a stop for step 11** — ask the BOX
FE team for the target's dummy label; never reuse Tier 1's on the strength of `PGT_SYS` being shared.
...
SELECT FK_INSTRUMENT, NUM_DAYS, DDATE, PK
...
| **2026-09-23** | Q-04c | **New, blocks step 4** — which `_X` column is the quote reference, `PK` or `FK_BS`; and the in-script source set |
