...
**Schema-qualified source names** `[stated: team runbook, 2026-09-15]`. The team's add-a-book runbook
([`../process/04-add-book-procedure.md`](../process/04-add-book-procedure.md) §1.2) names the same
sources with their schemas, as the in-conditions each load job waits on:
`cd_gcb_financial_formalised_contracts.{trade_details, pnl_leg, trade_details_sql, past_flows,
future_flows, position}` and `cd_sensibilidades.pnl`. These line up with the unqualified names in the
table above; `trade_details_sql` and the `cd_sensibilidades` schema appear only there. Note also that
the runbook's two Deal jobs run **serially** — the second waits on the first — rather than as
independent loads.
...
**Resolved, 2026-09-16 — the direction transformation is configured, not hardcoded.**
`Pay`/`Receive` → `Loan`/`Borrower` is a **`Direction` mapping row** in
`BOX_SYS.T_BOX_CROSS_REF_S`: `RECEIVE → BOR`, `PAY → LOA`, stated to be the pattern used by all
instruments, with the mapping filtered per instrument (mandatory `FK_INSTRUMENT_FILTER`). The
`SELL`/`BUY` variant seen on legs is the separate `Direction - Leg` type, landing on the same two GBO
codes. Full detail in [`tables/t-box-cross-ref-s.md`](tables/t-box-cross-ref-s.md)
`[stated: team page, 2026-09-16]`.

Two consequences. This is **configuration**, so it can differ per instrument and could in principle be
wrong or missing for one — a direction that doesn't resolve is a config gap with a specific row to go
look at, not a code bug. And the `[confirmed: code]` note above ("Deal direction maps Murex Pay/Receive
leg values to Borrower/Loan") is better read as *code applying this configured mapping* than as code
containing the mapping. The **cash-flow label** origin is still open.
...
`T_BOX_FLOW_DATA_S` and enriches processed flow values/classification. **It is step 2 of FE batch event
group `2629.65`** ([`job-chains/fe-batch-event-groups.md`](job-chains/fe-batch-event-groups.md)
§3), running directly after the Deal import and before the three `PKG_FE_PROPAGATION_DATA` procedures —
which is where the `PROPAGATION` column observed below is set. A same-date 27-AUG-26 join returns
...
**The transforming procedure is now named, 2026-09-16.**
`BOX_FE.PKG_FE_DEAL_CALCULATION.p_Import_Deal_Data` is step 1 of FE batch event group
`Import Deal Data And Flow Data BOX` (`2629.65`), immediately preceding the Flow equivalent below —
see [`job-chains/fe-batch-event-groups.md`](job-chains/fe-batch-event-groups.md) §3. That does
not supply the row-level join, but it converts the open question from "what performs this
transformation?" into "read this procedure", which is a much shorter route to closing it.

**Independently confirmed from the SIGOM side, 2026-09-15.** The shared-surface conclusion above was
drawn from row contents — one sample holding two products at once. It now has a second, unrelated
confirmation: every one of the 11 products under `BOX - Financial Engine > Financial Status` has its
own Deal Data *screen*, and a BOX FE Developer stated that all of them are backed by this same table
("even if each product has its view for Deal Data, they are all backed by the same table
`BOX_FE.T_BOX_DATADEAL_S`"). Contrast GBO, which has a genuinely separate `T_PGT_<CODE>DATAMIS_S` per
product — so the shared surface is a BOX-side design choice, not a convention inherited from GBO. Full
comparison in [`box-data-model.md`](box-data-model.md#financial-status-surfaces-gbo-vs-box-fe);
per-screen table names in [`sigom-reference.md`](sigom-reference.md).

Two independent confirmations make the shared-surface fact solid. The row-level RAW → DATADEAL join
above remains open regardless — knowing one table serves all products says nothing about how a RAW row
becomes a DATADEAL row.
...
> ⚠️ **Not a constant — BOX-DEV shows 2, not 30.** The add-a-product runbook
> ([`../process/05-add-product-procedure.md`](../process/05-add-product-procedure.md)) shows this screen
> in BOX-DEV with Swap, Commodity Swap and Deposit & Loan all at **2 days**. The "30 days for every
> observed instrument" below is a **Tier 1 production-copy** observation and remains accurate for that
> extract. Most likely an environment difference rather than a contradiction — but the practical point
> is that `NUM_DAYS` **varies by environment**, so neither figure should be carried into a new
> environment as a default. Adding a product also requires creating this row explicitly; it does not
> inherit. `[open-question]`
