...
> `scripts/render_sql.py` turns Q-04c's CSV into the `sys.odcinumberlist(...)` literal (refusing if it
> disagrees with Q-04c (b)'s counts or with the stated count); the validator checks the list equals the
> CSV; the block's self-check counts the rows written; and verify V4 compares the BOX array with GBO's in
> both directions. **The quote reference is
> `FK_BS`** — settled 2026-09-24; Q-04c is now the decoded, export-ready set, not a test between columns.
...
rows are **read from Q-06c's CSV by `scripts/render_sql.py`**, which lists every row it filtered out and
why in `03-sql/render-report.md`; checked by the validator (SQL = CSV) and verify V10 (BOX = GBO). The canonical query and the evidence: Q-06.
...
> instrument), plus one (**dummy book** × instrument) — dummy label `26391.4` in Tier 1. Row count
> `(books + 1) × instruments`: **12** for NY with one book. Every row's `FK_BRANCH` is `BRANCH_PK`.
>
> ⛔ **Tier 1's dummy does not exist in Tier 2** (run 4). The agent runs **Q-10d** to find candidates,
> then puts **two cards**: the branch's **books** (product SME) and the **Tier 2 dummy label** (BOX FE
> team). The dummy row **cannot be waived** — it is a BOX FE rule, not a scope choice — and the dummy is
> **never a real book's label**. No answer → step 11 `EVIDENCE_REQUIRED`, no SQL.
...
10. **The INSERT is not the whole operation — call the pre-commit procedure.** `[confirmed: DB,
    ...
    **Five of the walk's objects declare one**, covering steps 2, 3, 4, 5 and 6 — and probably 7–11,
    ...
    >
    > `[confirmed: source, 023-pkg_engprecommit_body.sql:395-443, 2026-09-24]` The COMMIT is **inside the
> loop** over the configuration's step-8 rows with a NULL curve: **no step-8 rows → no UPDATE, no
> COMMIT.** The rendered block therefore calls it and commits itself.
>
...
## The four SQL files — rehearsal, config, verify, rollback `[2026-09-23, rendered since 2026-09-24]`

**Run 3 had one file**, verification SELECTs absent and the undo inside `IF 1 = 0` — unreachable, so
neither correct nor tested (hard rule 11). Runs 4 and 5 had three, hand-written, each broken somewhere
new. **Since 2026-09-24 they are rendered** by `scripts/render_sql.py` from `03-sql/values.json`
([ADR 0006](../decisions/0006-render-sql-from-values.md)); the agent writes the values, never the SQL.

| File | Runs | Contains |
|---|---|---|
| `<BRANCH>-<env>-rehearsal.sql` | **first**, by the applying account, as often as needed | The config block, statement for statement — both pre-commits included — ending in `ROLLBACK` instead of `COMMIT`. Proves it compiles, every guard passes, every row inserts, every count is right and both pre-commits are callable — and keeps nothing. Last line `REHEARSAL OK` (plus any `WARNING`s) |
| `<BRANCH>-<env>-config.sql` | once, by the applying account — **`BOX_FE` only** | Refuses an open transaction; auth-code check before the first INSERT; step 0 pre-flight (no configuration for the branch, no DESCRIPTION collision, no step-11/12/14a rows); every step's header; `chk_pk()` after every `F___SEQUENCE` (fraction **and** PK unused); a self-check of every count; the curve pre-commit after step 4; the Config pre-commit after the self-check (a no-op with no step-8 rows, source-confirmed); a check the transaction is still its own; then `COMMIT`. Any error undoes the whole block. Last line `DONE - committed` |
| `<BRANCH>-<env>-verify.sql` | straight after, **read-only**, operator's account | The checks below. Every query returns **zero rows** (V3: one); anything else is a failure, named |
| `<BRANCH>-<env>-rollback.sql` | only to undo | Deletes in reverse dependency order, keyed on what this run created; **each delete must remove exactly the rows the config wrote**, or nothing is deleted; does not commit |

**What the verification script checks — each one a query with its expected result stated beside it:**

| # | Check | Expect |
|---|---|---|
| V1 | **Auth code** — every row this run created: `PK - TRUNC(PK) = <auth_code>/10^len` read from `gom_glb_sys.t__CORE_INFO_S` (which must have exactly one row) | 0 rows |
| V2 | Row counts per step (and one association for the branch) — only the steps whose count differs are listed | 0 rows |
| V3 | Header: this run's curve on `FK_CURVEMAN`/`FK_CURVEACC`; `FK_SOURCE_BACK = 586.4`, `FK_SOURCE_FRONT = <SOURCE_FRONT>`; calendar and currencies as mined | 1 row |
| V4 | Step 4 array = GBO array — `MINUS` both ways on `FK_BS` | 0 rows each way |
| V5 | Every branch written (steps 5, 7, 11) **equals `BRANCH_PK`**, and the association's is a real branch | 0 rows |
| V6 | Every instrument written is in `APPROVED_INSTRUMENTS` | 0 rows |
| V7 | Step 11: each approved instrument has its book rows **and** its dummy-book row | 0 rows |
| V8 | Days Matured: each approved instrument has a row | 0 rows |
| V9 | Identity: `FK_OWNER_OBJ`/`FK_EXTENSION` on every table written = the Q-G7 constants (NULL counts as wrong) | 0 rows |
| V10 | Step 7 = this branch's GBO exceptions for the approved instruments — `MINUS` both ways | 0 rows each way |
| V11 | The **decided** values (steps 6, 11, 12, 14a) as written = `values.json`, both ways | 0 rows each |
| V12 | Every reference value written exists: instruments, sources, book and dummy labels, quote references | 0 rows |
