...
| File | Runs | Contains |
|---|---|---|
| `<BRANCH>-<env>-rehearsal.sql` | **first**, by the applying account, as often as needed | The config block, ending in `ROLLBACK` instead of the committing pre-commit. Proves it compiles, every guard passes, every row inserts and every count is right — and keeps nothing. Last line `REHEARSAL OK` |
| `<BRANCH>-<env>-config.sql` | once, by the applying account — **`BOX_FE` only** | Refuses an open transaction; auth-code check before the first INSERT; step 0 pre-flight (no configuration for the branch, no DESCRIPTION collision, no step-11/12/14a rows); every step's header; `chk_pk()` after every `F___SEQUENCE` (fraction **and** PK unused); a self-check of every count; the curve pre-commit after step 4; the Config pre-commit last. Any error before it undoes the whole block. Last line `DONE - committed` |
