...
> | 3 | Don't list the quote refs; pick them from `DEVENG` | 4 | In-script cursor, count guard, set-equality check. **`PK` vs `FK_BS` unresolved** | Q-04c |
...
> **Step 4 reads its set in-script, never from a typed list** `[2026-09-23, expert point 3]`. A cursor
> over `DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = <GBO curve PK>`, a count guard that aborts if the set
> differs from the evidence, and a verification query comparing the BOX array with the GBO one in both
> directions. **Which column carries the quote reference is open** — the expert's query copies `PK`,
...
canonical query and the evidence: Q-06.
...
|---|---|---|
| `<BRANCH>-<env>-config.sql` | once, by whoever applies it | Auth-code check first (Q-G3c); every step's header; `chk_pk()` after every `F___SEQUENCE`; the count guard on every in-script set (step 4); pre-commits as today. **No `ROLLBACK`, no dead code** |
