...
| **Gate 0f** | ⛔ **NEW and blocking for this run.** Cross-tier FK resolution — Q-14. See below |
...
**⛔ Gate 0f is the catch, and it is the interesting part.** Every FK you mine from Tier 2 is a **Tier 2
PK**. Writing `FK_CALENDAR = 83.4` into Tier 1 is only correct if `83.4` names the same calendar there.
Run **Q-14** after mining and before generating any SQL: resolve every FK in *both* environments and
compare the descriptions. Three outcomes — resolves identically (use it), absent in Tier 1
(`EVIDENCE_REQUIRED`, no statement), or **resolves to a different object** (stop and escalate; that is
the silent-wrong-value failure this agent exists to prevent).

Per the auth-code finding, `.4` values were allocated in the global reference environment and are the
most likely to be valid in both tiers — `FK_CALENDAR = 83.4`, `FK_CURRENCY = 159.4` and the
Sub-Product instrument FKs are all `.4`. **That is a prior, not a conclusion.** Hard rule 8 applies:
check, don't assume.

**Two working assumptions, both stated and neither confirmed:** that `BOX_FE` table structures are
identical across tiers and differ only in content; and that `.4` reference data is genuinely replicated
rather than coincidentally numbered. Record both as assumptions. Q-14 tests the second directly.
