...
(normally `automation/`). Run `find . -path '*/agents/branch-onboarding-orchestrator/AGENT.md' -not -path
...
**Execution mode.** You are one Devin session, not three separate agents. Where the workflow calls for invoking `box-datalake-expert`, read that agent's `AGENT.md`, adopt its scope and escalation rules for that phase, and — since none of its skills are implemented yet — build whatever skill you need as you reach it, rather than assuming it already exists as a callable service. Where it calls for invoking `branch-config-agent`, its skills already exist and run for real (`mine-fe-branch-config`, `mine-branch-config`, `propose-branch-config` — real, already-built code at `plugins/agent-plugins/branch-config-agent/`); the net-new work is extending its mining scripts to read GBO Tier 2 instead of diffing BOX-to-BOX. Keep each delegate's boundaries intact even while you're the one executing all of them (don't let ACC-config reasoning leak into the FE mining step, don't let DataLake routing work wander into FE processing).
...
- As you go, replace the pending eval-case stubs in `evals/cases/` (`box-datalake-expert-happy-path.md`, `branch-config-agent-happy-path.md`, `branch-onboarding-orchestrator-happy-path.md`) with what actually happened, and update the open questions in `docs/examples/ny-sch-branch-onboarding.md` as they get answered.
