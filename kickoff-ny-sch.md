...
**Step 0 — find the repo root.** This automation repo usually sits in a subfolder of the checkout
(normally `automation/`). Run `find . -path '*/agents/sigom-box-fe-configs-agent/AGENT.md' -not -path
'*/archive/*'`, `cd` two levels above `agents/`, and treat every path below as relative to it. Never
report a file missing before running this search.
...
| `PRODUCT_BOOK_SCOPE` | 🟡 **Instruments answered 2026-09-21; the rest still blocking.** `[stated: <SME_NAME>, via Edouard]` NY_SCH gets **six**: Swap, Deposit & Loan, Cross Currency Swap, OTC Option, Caps And Floors, CDS (Credit Derivatives). That is **six of SLB's nine** — Cash Flow Matching, Forward Rate Agreement and Bond Return Swap are **out of scope**, so the "same set as SLB" hypothesis is **disproven** and must not be reinstated. Step 6 emits **six rows, not nine**; the same count governs steps 11 and 12. ⛔ **Still blocking:** the four `NOT NULL` accrual values per instrument (**24 values**), the `LIMIT_ERRORS` decision, and book scope — see [`runs/NY_SCH/tier2-pre/00-gate-0c-signoff-request.md`](../../../runs/NY_SCH/tier2-pre/00-gate-0c-signoff-request.md). ⛔ **CDS does not resolve to a `PGT_SYS.T_PGT_SUB_PRODUCT_S` row** `[confirmed: DB, 2026-09-18]`; resolve it with Q-05c's `LEFT JOIN` form plus the unjoined `COUNT(*)` before writing any row keyed on it, and never map it by name similarity. Confirming the **set** never authorises copying SLB's accrual **values** |
| `DB_ACCESS_MODE` | Ask at the start. If you don't have a read-only Tier 2 connection, run `assisted` |
| `SOURCE_ACCESS_MODE` | **`direct`** — you have access to `cib-boxfin-dbboxfe` and `cib-boxacc-dbboxacc`. **Use it.** Read a pre-commit body, a `CREATE TABLE`, a trigger, or the code that reads a table, rather than raising it as a blocker or reasoning about it. Record each question and answer in `05-source-questions.md` with a file:line citation. See the charter's *Source questions* |
...
**1. `FK_OWNER_OBJ` and `FK_EXTENSION` are never mined from GBO.** `FK_OWNER_OBJ` is the **screen the
row is edited through**, `FK_EXTENSION` is **which tab within it** — properties of the destination.
Every `DEVENG.T_PGT_ENGCONF_S` row carries `12198.4`, the GBO Financial Engine's own object. Your
`INSERT … SELECT … FROM DEVENG.T_PGT_ENGCONF_S` copied that into a BOX row.

**And it is not one constant across the walk.** The walk spans **three screens** — `35000126.65` Config
(steps 2, 5, 6, 7, 8, 9, 10, 11), `35000123.65` FixingCurve (steps 3, 4), `35000289.65` Limit Error
Assign (step 12). Applying one value everywhere is wrong for steps 3, 4 and 12.

**Derive them from Q-G7**, which reads `GOM_GLB_SYS` and needs no `BOX_FE` access — base table → the
object's PK with a NULL extension; extension table → the extension's `FK_PARENT` with the extension's
own PK. This is off the deferred list now, so bind real constants rather than variables.
...
`LIMIT_ERRORS` values (115 for CCS, 100 for the rest) were recorded as confirmed / "User decision"
without a name or a date.

⚠️ **An SME has since confirmed six instruments by name — and that does *not* retrospectively validate
those PKs.** `[stated: <SME_NAME>, via Edouard, 2026-09-21]` The confirmed set is Swap, Deposit & Loan,
Cross Currency Swap, OTC Option, Caps And Floors, CDS. The draft's six PKs may resolve to that same
set, a different set, or overlap partially — **nobody has checked, and the count matching is not
evidence.** A list that was unattributed when written does not become attributed because a later,
independent answer happens to have the same length.

**So: resolve the six confirmed names to `PGT_SYS.T_PGT_SUB_PRODUCT_S` PKs from the data** (Q-05c,
`LEFT JOIN` form), then compare against the draft's list and report the diff as a finding. Do not adopt
the draft's PKs.

✅ **Expect all six to resolve.** `[confirmed: DB via Edouard, 2026-09-22]` Credit Derivatives is
**`20313.4`** (CODE `Credit`). An earlier version of this prompt said CDS had no Sub-Product row and
that at most five could resolve — **both withdrawn**; that was inferred from an inner join that failed
to match, which is a fact about the join.

⚠️ **The draft listed `20314.4`** — one away from `20313.4`. Check whether it mis-keyed Credit
Derivatives by one. A hypothesis to test, not a conclusion.
...
**~~A challenge to the branch-keyed model itself.~~ ✅ Closed 2026-09-18 — the walk's grain is correct.**
BOX-DEV screenshots showed the Accrual Exceptions and Allowed Errors `Branch` column holding
product-shaped values (`BOX CCS`, `BOX FX`, `BOX IRS`), which would have changed the walk. The metamodel
settles it: `BOX_ENG_Config_Accrual.pBranch` and `BOX - Limit Error Assign.pBranch` both declare
**`PGT - Branch` → `PGT_STC.T_PGT_BRANCH_S`**, the geographic branch master. Those values are rows
*inside* that master, which this repo already records as overloaded — ~137 rows including entities,
SPVs, counterparties and test records. **Do not re-open this or stop the run over it.**

## 🆕 Gate 0g — run the metamodel read before the walk

`GOM_GLB_SYS` declares what this repo has been inferring. **Q-G7 needs no `BOX_FE` access**, so run it
first, every run. It gives you three things:

1. **The identity constants** per target table (point 1 above).
2. **The declared FK target of every column you write.** `pInstrument` → `T_PGT_SUB_PRODUCT_S` on six
   objects and → `V_BOX_PROC_INSTR_S` on the two **fixing** objects (steps 8, 13). `pBranch` →
   `T_PGT_BRANCH_S` everywhere. `pLabel` → `PGT_DOMAINS`. **Never infer a join this query declares.**
3. **A completeness diff.** It already found one: `BOX_ENG_FixingCurve.apYieldCurve` →
   `T_BOX_ENGFIXDISC_S`, a declared tab with no walk step. Now **step 4b**, `EVIDENCE_REQUIRED`, parked
   with steps 9/10, no INSERT — it is empty in Tier 1 PRE, which is not evidence it is unneeded. Do not
   confuse it with step 9's Yield Curve (`T_BOX_ENGZCCONF_S`); different object, different level.

It also confirmed Q-08's and Q-09's `FK_PARENT` — both are declared owned collections of
`BOX_ENG_Config`, so `FK_PARENT = 64408.35` is right for them after all. (They stay parked for the
separate reason that nobody has said whether NY needs rows.) Reference:
`docs/reference/sigom-metamodel.md`.

## ⛔ Pre-commit procedures — the draft cannot execute until these are read

`[confirmed: DB, 2026-09-18]` **Five walk objects declare code that runs on save**, covering steps 2, 3,
4, 5 and 6:
...
So a raw INSERT is **not** the whole operation for the spine of the walk. Each takes `(pk IN NUMBER)` —
the PK of a row that already exists — and is `AUTHID DEFINER`, so no extra grants are needed.

### ✅ The bodies have been read — 2026-09-21 — and they change your output

| Procedure | Verdict | What you do |
|---|---|---|
| `P_ENGFixingCurve_PreCommit` | **No DML.** Validates that if any currency has a yield curve, every currency in the quote reference has one; raises `-20001` | Call it after steps 3/4 as a gate. Safe |
| **`p_check_Val_Curves_precommit`** | ⛔ **Writes to step 8's table, and `COMMIT`s** | See below |

**Passed the step-2 header PK, `p_check_Val_Curves_precommit` UPDATEs
`T_BOX_FIXING_BY_INSTR_S WHERE fk_parent = pk_in`** — back-filling null `fk_fixingcurve_acc` /
`fk_fixingcurve_man` from the header defaults — and issues an explicit **`COMMIT`** after each of its
three conditional UPDATEs.

**Three things follow. Apply all of them:**

- **Your trailing `ROLLBACK` is not a safety net.** A committing procedure commits everything
  outstanding, including every INSERT so far. The rollback `DELETE`s are the real undo. Never describe
  the script as "run it and roll back" — charter hard rule 11.
- **Emit step 8's rows with their curve values populated.** Where the GBO row is null, fill from the
  configuration header and tag it **`DERIVED`**, citing `023-pkg_engprecommit_body.sql:413-438` as the
  rule.
- **Then call the procedure as a check.** If it updates nothing it commits nothing — and finding nothing
  to do is positive evidence your INSERT set already matched SIGOM. That is procedure step F3, solved.

## 🆕 Four more things from the source round — apply these to the next draft

**1. There are no `FOREIGN KEY` constraints anywhere.** Not one, across all thirteen tables. A wrong FK
**inserts cleanly** and fails silently later, in a batch, far from the cause. Nothing will catch it for
you — Q-G7's declared targets are the only defence that exists. Never infer a join.

**2. `DESCRIPTION` is globally unique** on `T_BOX_ENGCONF_S` and `T_BOX_ENGFCURVE_S` — across the whole
table, not per branch. Before emitting steps 2 and 3, check the mined description does not already exist
in the target. A hit is `SME_DECISION_REQUIRED` on naming, never a value you adjust quietly.

**3. Step 6 has four mandatory *values*.** `FK_FEEFIRSTDAYSEL`, `FK_INTFIRSTDAYSEL`, `INTCOMMONBASIS`
and `BYTRIGGER` are `NOT NULL`, so they cannot wait for an SME. **Gate 0c is "which instruments **and**
these four values for each"** — put it to the SME that way.

**4. Step 12 is more serious than you were told.** `LIMIT_ERRORS` defaults to **`0`** when no row exists
for a `(FK_BRANCH, FK_INSTRUMENT)`, so the first failed deal aborts the entire load. One row per
in-scope instrument is **mandatory**. The value is a per-run tolerance for failed deals, not cumulative.

🔄 **And its source changed, 2026-09-21** `[stated: Edouard]` — **do not mine the GBO twin.** Run Q-13
against **SLB** and propose SLB's limit for each of NY_SCH's six instruments, same instrument one for
one, tagged **`PROPOSED`** for the SME to accept or change.
[ADR 0004](../../../docs/decisions/0004-step12-limits-from-reference-branch.md) explains why that is
legitimate here and **nowhere else** — it does not extend to step 6's four values, and an instrument
SLB lacks is `SME_DECISION_REQUIRED`, never the nearest-looking instrument's number.

**And two statuses change:** steps 9, 10 and 4b are ordinary SIGOM-UI configuration — nothing in either
repo writes them, so they are neither derived nor batch-populated. They stay `EVIDENCE_REQUIRED`, but
the question for the SME is now *"does NY need these?"*, not *"what are these?"*

## Non-negotiables

- **Gates first, in order.** Q-G7 *(gate 0g — run it first, it needs no `BOX_FE` access and it
  parameterises the INSERTs)* → Q-G1 → Q-G2 *(level 1 only)* → Q-G3 → Q-G4 → Q-G5 → Q-G6. Nothing in the
  walk is valid until they pass,
