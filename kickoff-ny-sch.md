...
| `GBO_SOURCE` | GBO **Tier 2** — **optional as of 2026-09-16**, for cross-reference only. Do not mine GBO for instrument configuration and do not gate on it (see the model correction below) |
| `PRODUCT_BOOK_SCOPE` | **Not yet supplied — blocking.** Which of the instruments **already configured in BOX** NY_SCH gets: all, or a named subset. From a named SME in writing. The working hypothesis is "the same set as SLB" — that is a hypothesis to confirm, never an input to assume |
| `REFERENCE_BRANCH` | **SLB** — the existing BOX branch used as the structural analogue. Shape only; copying its values is a hard-rule violation |
...
## ⚠️ Model correction, 2026-09-16 — this supersedes part of your earlier run

`[stated: BOX Developer via Edouard, 2026-09-16]`

If you already started this run and blocked on **gate 0b**, that block is withdrawn. Read the
correction block at the top of `AGENT.md` in full. In short:

- **Do not mine GBO for instrument configuration.** It is not the source.
- **BOX already holds the instrument configuration.** Onboarding NY_SCH means attaching it to all, or
  an SME-chosen subset, of what BOX already has.
- **Within your scope, the work is principally MIS and the Fixing Curve** — walk steps 2–5. The rest
  of the *FE config walk* either follows automatically or is verification. This says nothing about
  Jobs, BOX ACC config, Reporting, GL or FDH — those are other workstreams, unchanged and still
  required. Do not report NY_SCH as onboarded because your walk finished.
- **Gate 0b (Q-G2) is withdrawn.** It was both unnecessary *and* broken: its child-level join returns
  nothing for Madrid either, so your empty result was a query defect, not a finding about NY_SCH.
  Do not spend further effort on it.
- **Gate 0b′ (Q-G5) replaces it** — read `REFERENCE_BRANCH`'s FE objects in BOX to establish shape.
  Unlike Q-G2, this query *must* return rows; an empty result means the query or the branch choice is
  wrong, never that the branch is unconfigured.

**One thing to flag, not resolve:** the Developer said "the rest will automatically be updated." That
sits against an earlier BOX FE Developer statement (2026-09-11) that a `T_BOX_CONF_BY_BOOK_S` row is
what registers a `(branch, instrument)` pair for the batch to execute at all. Walk step 11 therefore
stays in scope, marked `SME_DECISION_REQUIRED`, until someone reconciles the two. Record it in
`99-open-items.md` as the top item; do not pick a side.
...
- **Gates first, in order.** Q-G1 → Q-G5 → Q-G3 → Q-G4. *(Q-G2 removed 2026-09-16.)* Nothing in the walk is valid until they pass,
