...
**Fresh evidence for that hypothesis, from a `BOX_FE` table rather than GBO.**
`[confirmed: DB via Edouard, 2026-09-17]` SLB's nine Accrual rows carry these `InternalID` values
(rendered in the Spanish locale the client uses — `.` is the thousands separator, `,` the decimal):
`24.095.416,21` · `19.146.735,21` · `2.884.060,21` · `22.085.977,21` · `44.552,21` · `31.874.132,21` ·
`23.029.079,21` · `26.216.212,21` · `31.874.121,21`. Three things follow, all `[inferred]` and all
cheap to test: the `<integer>.<authcode>` shape holds **inside `BOX_FE`**, not just GBO; all nine share
suffix `.21`, consistent with one owning authority per configuration; and the integer parts are large,
scattered (44,552 up to 31,874,132) and in one case 11 apart — which looks like a **sequence shared
across many tables** rather than a per-table one. If so, one `ALL_SEQUENCES` row with `LAST_NUMBER`
above ~31.9 M is the allocator. Confirm it; do not assume it, and do not use it to construct a PK
before gate 0d is signed off.
