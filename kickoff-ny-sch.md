...
1. **Call the procedure after each affected INSERT**, with the PK variable you already declared:
   `PKG_ENGPRECOMMIT.p_check_Val_Curves_precommit(v_pk_engconf);`
2. **Get the body read before anything executes.** It is committed in `cib-boxfin-dbboxfe` — you have
   repo access. Report, per procedure: does it perform any DML, or is it reads plus
   `RAISE_APPLICATION_ERROR`? Quote any DML. `ALL_SOURCE` returns only the 16-line spec; that is a
   privilege artifact, not a small procedure.

If it validates, the script reproduces SIGOM's check. If it writes, the walk is incomplete by exactly
that much and you report it as a finding.
