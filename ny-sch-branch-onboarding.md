...
| **W2** | **SIGOM configs** | BOX FE configs (the eight-tab MIS aggregate, fixing curves, …) and BOX ACC configs (portfolio properties, topics, GL accounts, …). **A tool already exists that maps GBO Topics → BOX Topics** and should be used rather than re-deriving that mapping. **One qualification, 2026-09-16** `[stated: BOX Developer]` — FE configs are still mined from their GBO equivalents; the exception is **instrument scope**, chosen from what is already live in BOX by a named SME rather than read from the branch's GBO instrument config. See [`sigom-box-fe-configs-agent/AGENT.md`](../../agents/sigom-box-fe-configs-agent/AGENT.md) | `documented` (FE), `partial` (ACC) | not started |
...
- **Corrected 2026-09-16 from the first live Tier 2 run (Q-G1).** `20007.4` is NY_SCH's **branch PK**,
  not its entity — this document previously mislabelled it. The confirmed Tier 2 identity row is:
  `BRANCH_PK = 20007.4`, `FK_ENTITY = 31398.4`, `FK_CURRENCY = 159.4`, `FK_CALENDAR = 83.4`,
  `FK_LOCALGROUP = 21447.4`. `[confirmed: DB Tier 2, 2026-09-16]`
- **`FK_LOCALGROUP = 21447.4` is a real input to open question 4** (below): it differs from Madrid's
  `269.4`, so NY_SCH does not share Madrid's branch group. Whether it shares SLB's, or is new, is one
  query away.
- NY_SCH's GBO branch-config row and FE/MIS header both exist in Tier 2: `bc.PK = 141.35`,
  `FK_MISCONFIG = 64408.35` → resolves to `DEVENG.T_PGT_ENGCONF_S`, "Configuracion -NY".
  `[confirmed: DB Tier 2, 2026-09-16]` Under the 2026-09-16 model correction this is **orientation, not
  a prerequisite** — BOX FE onboarding does not read GBO's instrument configuration.
...
1. **Tier 2 DB access** — until this exists, nothing below can move from "proposed" to "confirmed." A
   ...
   **↳ Sharpened 2026-09-16** `[confirmed: DB]` — this is now **two** distinct asks, and only one is
   satisfied. The Tier 2 account in hand reads GBO (`DEVENG`) fine; it has **no grants on `BOX_FE`**.
   That single fact produced the withdrawn "Tier 2 PRE is missing a substantial number of `BOX_FE`
   tables" finding — an account without privileges sees no rows in `ALL_TABLES`, indistinguishable from
   absence — and with it the false alarm about whether `BOX_FE` was ever deployed to Tier 2 at all.
   Both are stood down. **What is actually needed is a Tier 2 account with `BOX_FE` grants**; ask for
   that specifically rather than for "Tier 2 access," which now reads as already granted. Meanwhile the
   FE walk continues as a Tier 1 rehearsal — see `runs/README.md`.
...
5. **Explicit product scope for NY_SCH**, from the business/FO SME — never inferred from what
   ...
   products.
   **↳ Sharpened 2026-09-16** `[stated: BOX Developer via Edouard]` — the question is now easier to
   ask, because the answer does not come from GBO. A branch is onboarded with **all, or a subset of,
   the instruments already created and live in BOX**, so the SME question is simply: *"Which of BOX's
   live instruments does NY_SCH get — all of them, or which subset?"* The working hypothesis is **the
   same set as SLB**, to confirm rather than assume. The menu is the BOX FE Processed Instruments
   catalogue in `../reference/box-data-model.md`. (This concerns instrument scope only — the MIS config
   and Fixing Curve are still mined from GBO.)
   **↳ Now answerable, 2026-09-17** `[confirmed: DB via Edouard]` — the "same set as SLB" hypothesis
   stops being unfalsifiable. A branch's instrument set is read from the MIS configuration's **Accrual
   tab**, `BOX_FE.T_BOX_ENGACCRCONF_S`, one row per instrument (the Processed Instruments catalogue is
   the *menu*; the Accrual tab is the *selection*). **SLB in Tier 1 has nine:** OTC Option, Cross
   Currency Swap, Swap, Cash Flow Matching, Deposit & Loan, Credit Derivatives, Forward Rate Agreement,
   Caps And Floors, Bond Return Swap. So the SME question becomes concrete — *"NY_SCH gets these nine,
   or which subset?"* — which is a far better question than asking someone to enumerate from memory.
   **↳ 2026-09-18:** the SQL read of the same configuration returns only **seven** — Credit Derivatives
   and Bond Return Swap don't resolve to a `T_PGT_SUB_PRODUCT_S` row, and an inner join dropped them.
   Use the nine from the screen, query with a `LEFT JOIN` (Q-05c), and run the unjoined `COUNT(*)` to
   settle it. Confirming the set does **not** authorise copying SLB's accrual values.
   **↳ ✅ ANSWERED 2026-09-21** `[stated: <SME_NAME>, via Edouard]` — NY_SCH gets **six**: Swap,
   Deposit & Loan, Cross Currency Swap, OTC Option, Caps And Floors, CDS (Credit Derivatives).
   **The "same set as SLB" hypothesis is disproven** — Cash Flow Matching, Forward Rate Agreement and
   Bond Return Swap are out of scope. Had the walk assumed it, three unrequested instruments would have
   been configured. Record this as the case for asking rather than assuming, and note the asymmetry:
   the hypothesis was *plausible, cheap to test, and wrong*.
   **↳ ✅ CORRECTION 2026-09-22** `[confirmed: DB via Edouard]` — an earlier note here said the picked
   set included CDS as an instrument with no Sub-Product row, blocking steps 6, 11 and 12. **Withdrawn.
   Credit Derivatives is `20313.4`, CODE `Credit`, and all six instruments resolve.** The 09-18
   evidence was an inner join that failed to match; reading that as "the instrument is absent" was an
   inference tagged `[confirmed]` and propagated across ten files. What remains open is only *why the
   join missed* — compare SLB's accrual `FK_INSTRUMENT` against `20313.4`. **Nothing is blocked by it.**
   **Gate 0c is still blocked**: instruments are one of three answers it needs (the 24 mandatory
   accrual values and the `LIMIT_ERRORS` / book-scope decisions are outstanding).
   **Earlier framing, still true as background (2026-09-15):** BOX FE's Financial Status covers **11
   products** against GBO's ~28 — see `../reference/box-data-model.md`. A dozen GBO products (Repo,
   Futures, ETD, Lending, SCF, the four equity families, FIFW/FISP, ES) have **no BOX FE counterpart at
   all**, and GBO's four FX products and five OTC sub-products each collapse into a single BOX product.
   So the scope question is no longer just "what does NY trade" but "what does NY trade **that BOX FE
   can represent**" — and any answer landing outside those 11 is a product-coverage gap for the BOX
   programme to decide on, not a configuration task the FE agent can mine its way through. Check the
   SME's answer against that list at the moment it arrives, not after the walk has started.
