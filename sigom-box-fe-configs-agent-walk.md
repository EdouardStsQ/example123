...
- **An INSERT emitted for an object with an unread pre-commit procedure** — five walk objects declare
  one, covering steps 2–6. The script calls it after each affected INSERT (hard rule 10), and the body
  is read from `cib-boxfin-dbboxfe` before anything executes. `ALL_SOURCE` returning only 16 lines is
  the spec, not the package — treating that as "the procedure is trivial" is hard rule 8 in a new
  costume.
...
- **A field's target assumed from its name** — `pBranch` resolves to `T_PGT_BRANCH_GROUP_S` on two ACC
  objects and `T_PGT_BRANCH_S` on twelve. For NY_SCH those are `21447.4` and `20007.4`: both valid PKs,
  and the wrong one fails silently. Resolve per object from Q-G7.
