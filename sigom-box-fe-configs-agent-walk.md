...
**Part 3 — config statements.** INSERTs in the order **2→3→4→5→6→7→8→11→12**, each annotated with the
...
step 14 (not branch-scoped) — and none for **steps 4b, 9 and 10**, which are `EVIDENCE_REQUIRED` and
parked pending the question of whether this branch needs them. That is the order
`scripts/validate_run_output.py` enforces; a run that re-includes 9 and 10 must have closed their
question first. **Every PK is `F___SEQUENCE('<TABLE>','X')` assigned to a declared variable**, never a
...
- **A curve created and never referenced** — steps 2–4 are **insert-then-update**. A run that INSERTs
  `T_BOX_ENGFCURVE_S` without an `UPDATE` of `T_BOX_ENGCONF_S` setting `FK_CURVEMAN`/`FK_CURVEACC` to
  that curve's variable has built a configuration pointing at no curve it created. Equally a fail:
  leaving those columns NULL at the end of the run.
