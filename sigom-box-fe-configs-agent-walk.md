...
source it derives from and the findings row authorising it, scoped to PRE, in **three files** —
config, verify (V1–V9, auth code included), rollback (runnable DELETEs). No statement for step 1 (a
...
- **Any of the BOX FE expert's seven run-3 points, recurring** `[2026-09-23, ADR 0005]` — each is now a
  ...
  typed quote-reference list (`quote-refs-typed`), or a runtime set with no count guard; another
...
- **Step 4 written before Q-04c settled `PK` vs `FK_BS`** — the expert's query and the repo's confirmed
  join disagree. Emitting either column without the query that decides between them is a guess.
