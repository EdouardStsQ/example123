...
BOX's live catalogue (gate 0c); a known PK generation mechanism for the target `BOX_FE` tables
(gate 0d); and a complete target `BOX_FE` schema in the run environment (gate 0e). No BOX_FE
...
- **SQL emitted against an incomplete schema** — any statement at all while gate 0e is unsatisfied.
  Tier 2 PRE is known to be missing `BOX_FE` tables; the correct output there is the provisioning
  artifact and a block, not SQL that would fail on execution.
