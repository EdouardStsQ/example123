...
| `BOX_ENG_Setup` | FE step 14b (nothing to configure) | `P_FE_Parameters_PreCommit` |
...
> ### ⛔ Bodies read 2026-09-21 — classify before calling
>
> An earlier version of this section said *"the script calls the procedure after each affected INSERT"*
> and offered a snapshot-and-rollback test. **Both are wrong for one of these procedures.**
>
> **`PKG_ENGPRECOMMIT.p_check_Val_Curves_precommit`**, passed the step-2 header PK, UPDATEs a
> **different table** — `T_BOX_FIXING_BY_INSTR_S WHERE fk_parent = pk_in`, back-filling null fixing
> curves from the header defaults — and issues an explicit **`COMMIT`** after each of its three
> conditional UPDATEs. `P_ENGFixingCurve_PreCommit`, by contrast, does **no DML** at all.

Three classes, and the handling differs for each (charter hard rules 10 and 11):

| Class | Handling |
|---|---|
| **No DML** | Call it after the INSERT as a validation gate. Safe anywhere |
| **Same-row DML** (the ACC procedures) | Don't supply the columns it computes; call it, then read the row back |
| ⛔ **Cross-table DML + `COMMIT`** | **Never inside a transaction you intend to roll back.** A commit inside the procedure commits every INSERT outstanding, so the script's trailing `ROLLBACK` is not an undo |

**Procedure step F3's method depends on that class.** Snapshot → INSERT → call → re-snapshot →
`ROLLBACK` is valid only for a **no-DML** procedure. For the committing one the method is instead
**make it a no-op and call it**: populate step 8's curve values at INSERT time so its UPDATEs never
fire — then it commits nothing, and finding nothing to do is itself the evidence that the INSERT set
matched SIGOM.
...
column — Setup and Days_Matured declare no branch field at all. ⚠️ **Refined 2026-09-23:** no branch
field is not the same as nothing to do — Days Matured is keyed by *instrument*, so an approved
instrument without a row gets one (step **14a**). Setup is step **14b**, nothing to configure.
...
## 6c. Why this page is load-bearing: there are no FK constraints

`[confirmed: DDL via Devin, 2026-09-21]` Across all thirteen FE walk and ACC configuration tables there
is **not one `FOREIGN KEY`, `CHECK` or `DEFAULT` clause.** Every `FK_*` column is a plain `NUMBER`.
Referential integrity is **entirely application-layer**.

Three consequences:

1. **`T__EXT_DEF_S` is the only source of truth for the join graph.** There is no dictionary copy to
   check against, no `ALL_CONSTRAINTS` fallback. That is why every relationship in this project has had
   to be inferred, and why gate 0g is a gate rather than a nicety.
2. **A wrong FK inserts cleanly.** Nothing rejects it. It fails silently later, in a batch, far from the
   cause — which is precisely the failure mode the whole agent exists to prevent. Getting the target
   right *before* the INSERT is the only defence that exists.
3. **The `pBranch → Branch Group` trap is unguarded.** `20007.4` where `21447.4` belongs would be
   accepted without complaint.

**What the DDL *does* constrain**, and an agent must respect:

| | |
|---|---|
| `NOT NULL` column sets | the minimum INSERT column list per table |
| Unique indexes | the natural keys — and two of them are **global**: `T_BOX_ENGCONF_S.DESCRIPTION` and `T_BOX_ENGFCURVE_S.DESCRIPTION`. A duplicate description is a hard INSERT failure |

⚠️ **`T_BOX_LINK_ARRAY_X` is a `BOX_ACC` table, not `BOX_FE`** — and its DDL shows `FK_OWNER_OBJ` and
`FK_EXTENSION` are `NOT NULL` but sit only in a **non-unique** header index
(`FK_PARENT, FK_OWNER_OBJ, FK_EXTENSION`). The PK constraint is on `PK` alone. So the discriminator
model is confirmed structurally — those columns are mandatory and are the lookup path — while **nothing
at the database level prevents cross-array contamination.** Consistent with integrity being
application-layer, and one more reason the `(FK_OWNER_OBJ, FK_EXTENSION, FK_PARENT)` filter is on the
query author.

Two anomalies: **`T_BOX_CONFIG_ACCRUAL_S` (FE walk step 7) has no primary key and no `NOT NULL`
columns** — uniqueness rests on five business columns, so whether it even needs an `F___SEQUENCE` PK is
`[open-question]`. And `T_BOX_ACCT_PORT_PROP_S` / `T_BOX_ACCT_LIST_TOPIC_S` have only non-unique
secondary indexes beside their PK.

*Caveat: read from the initial `CREATE TABLE` per table. A constraint added by a later `ALTER TABLE`
changeset would not appear. Given SIGOM's all-integrity-in-PL/SQL pattern, unlikely — not impossible.*

## 6d. Grants — ask for these by name

`[confirmed: deployment scripts via Devin, 2026-09-21]` Both repos grant through Liquibase scripts under
`25_Grants/`. **No `CREATE ROLE` exists in either** — the roles pre-exist and are granted *to*.

| Schema | Read role | Write role |
|---|---|---|
| `BOX_FE` | **`BOX_FE_RD`** — `SELECT` on tables/views, `EXECUTE` on packages | **`BOX_FE_WR`** — `SELECT, INSERT, DELETE, UPDATE` on tables, `EXECUTE` on packages |
| `BOX_ACC` | **`BOX_ACC_RD`** — `SELECT` / `EXECUTE` | **`BOX_ACC_WR`** — full DML (sometimes `ALTER`), `EXECUTE` on packages, `SELECT` on sequences |

Cross-schema: `BOX_ACC` gets `SELECT` (occasionally `UPDATE`) on FE data; `PGT_PRG`/`PGT_PRC`/`PGT_MRK`/
`PGT_STC`/`PGT_SYS` get `SELECT` on selected FE objects; `BOX_TRD` gets `SELECT` on a few tables;
`ROL_APLICACION_EXTRACTOR` gets `SELECT` on the FDH views. In the ACC repo the grant runs the other way
too — `PGT_PRG.Pkg_Pgterror` is granted `EXECUTE` **to** `BOX_ACC`.

**Two things this settles.**

**The access request has names now.** Instead of "Tier 2 `BOX_FE` access" — which reads as
already-granted, because for GBO it is — ask for **`BOX_FE_RD`** for the agent's mining and verification,
and **`BOX_FE_WR`** for whoever executes the script. Those are the roles the deployment itself defines.

**`F___SEQUENCE` comes with them.** It is granted as part of the standard object set alongside tables,
views, packages and `ENG_GN_GLOBALVAR` — so an account holding `BOX_FE_WR` can call it, and gate 0d's
Tier 2 reachability question (Q-G3b) is answered by the same grant.

Also: **`cib-boxfin-dbboxfe` creates no synonyms at all** (ACC creates three, for MTM tables only). So
every FE query must be schema-qualified — `BOX_FE.T_BOX_…` — which the catalogue already does.
