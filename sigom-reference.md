...
- **GBO** (top-level node)
  ...
  Of GBO's second-level children, only **Accounting** and **Financial Engine** have been expanded in
  full. **Static Data** is now partially expanded — one path through it is confirmed
  `[confirmed: DB via BOX FE Developer, 2026-09-15]`:

  - Static Data
    - Environment
      - Config
        - **Folders** (leaf) → `PGT_STC.T_PGT_FOLDER_S` — branch-keyed; see
          [`box-data-model.md`](box-data-model.md#folders--branch-keyed-gbo-static-data)

  Only this one path is known; `Environment`'s and `Config`'s other children, and Static Data's other
  children, are still unexplored. The remaining second-level children are untouched — their contents
  are open questions.
...
| BOX - Financial Engine > Process Management > Allowed Errors | Allowed Errors | `BOX_FE.T_BOX_ERRORS_FE_S` `[stated: BOX FE Developer, 2026-09-17]` — error limit before a process crashes, defined **per instrument and per branch** |
...
| BOX - Financial Engine > Financial Status > *(all 11 products)* | Deal / Financial / MtM Data | see the per-product table in *All 11 BOX products, confirmed by name* below |
| BOX - Financial Engine > Static IT Data > Processed Instruments | Processed Instruments | `BOX_FE.T_BOX_ENGINSTRUMENTS_S` |
| BOX - Financial Engine > Static IT Data > *(several codes-config leaves, e.g. Deal Status)* | codes/domain screens | `BOX_FE.T_BOX_ENGDOM_S` — **one table behind several leaves**; see note below |
| GBO > Financial Engine > Financial Status > *(all products)* | Deal / Financial / Marketvalue and Risk / MIS-MDR Interface | see *GBO's `Financial Status`, for comparison* below |
| GBO > Financial Engine > Static MIS Data > Branch Mis | Branch Mis | `DEVENG.T_PGT_BRANCH_MIS_S` |
| GBO > Financial Engine > Static MIS Data > MISProduct | MISProduct | `DEVENG.T_PGT_ENGMISPRODUCT_S` |
| GBO > Financial Engine > Static MIS Data > Processed Instruments | Processed Instruments | `DEVENG.T_PGT_ENGINSTRUMENTS_S` |
| GBO > Financial Engine > Static MIS Data > *(same codes-config leaves)* | codes/domain screens | `DEVENG.T_PGT_ENGDOM_S` |
| GBO > Static Data > Environment > Config > Folders | Folders | `PGT_STC.T_PGT_FOLDER_S` — branch-keyed; query and columns in [`box-data-model.md`](box-data-model.md#folders--branch-keyed-gbo-static-data) |
...
- `BOX <PRODUCT> Financial Data` → `BOX_FE.T_BOX_<CODE>FINANCST_S` (product-specific table).
- `BOX <PRODUCT> MtM Data` → `BOX_FE.V_BOX_ENG<CODE>DATAMIS_S` — note the `V_` prefix (a **database view**, not a table, unlike the other two), and the `ENG` infix (consistent with the `ENG` = "Engine" pattern already seen under Control > Configuration).

### All 11 BOX products, confirmed by name `[confirmed: DB via BOX FE Developer, 2026-09-15]`

The caveat that previously stood here — that only the CCS instantiation had been confirmed and the other ten were derived from the stated pattern — is now **resolved**. Every product's Financial Data table and MtM view has been read directly:

| Financial Status folder | `<CODE>` | Financial Data table | MtM Data view |
|---|---|---|---|
| BRS | `BRS` | `T_BOX_BRSFINANCST_S` | `V_BOX_ENGBRSDATAMIS_S` |
| C&F (Cap & Floor) | **`CF`** | `T_BOX_CFFINANCST_S` | `V_BOX_ENGCFDATAMIS_S` |
| CCS (Cross Currency Swap) | `CCS` | `T_BOX_CCSFINANCST_S` | `V_BOX_ENGCCSDATAMIS_S` |
| CDS (Credit Default Swap) | `CDS` | `T_BOX_CDSFINANCST_S` | `V_BOX_ENGCDSDATAMIS_S` |
| CES | `CES` | `T_BOX_CESFINANCST_S` | `V_BOX_ENGCESDATAMIS_S` |
| CFM | `CFM` | `T_BOX_CFMFINANCST_S` | `V_BOX_ENGCFMDATAMIS_S` |
| Depo (Deposits) | **`MM`** | `T_BOX_MMFINANCST_S` | `V_BOX_ENGMMDATAMIS_S` |
| FRA | `FRA` | `T_BOX_FRAFINANCST_S` | `V_BOX_ENGFRADATAMIS_S` |
| FX | `FX` | `T_BOX_FXFINANCST_S` | `V_BOX_ENGFXDATAMIS_S` |
| OTC | `OTC` | `T_BOX_OTCFINANCST_S` | `V_BOX_ENGOTCDATAMIS_S` |
| Swap | **`IR`** | `T_BOX_IRFINANCST_S` | `V_BOX_ENGIRDATAMIS_S` |

All in schema `BOX_FE`. Deal Data for every one of the 11 is `BOX_FE.T_BOX_DATADEAL_S` — confirmed as a blanket statement by the developer ("even if each product has its view for Deal Data, they are all backed by the same table"), not extrapolated from CCS.

**Correction to the rule as previously written:** `<CODE>` is a **product code, not the folder label**, and the two differ for 3 of the 11 — `C&F`→`CF`, `Depo`→`MM`, `Swap`→`IR` (bolded above). The earlier phrasing `T_BOX_<PRODUCT>FINANCST_S` would produce wrong table names for those three. Read the code from this table rather than deriving it from a screen name.

**`FX` is provisioned but not live** `[stated: BOX FE Developer, 2026-09-15]` — the folder and its tables exist, but no trades have been booked and FX go-live is still pending. An empty `T_BOX_FXFINANCST_S` therefore means "not yet live", not "misconfigured".

The shared-Deal-Data half of the rule also remains independently corroborated at the DB level: `fe-raw-data-stage.md` sampled `T_BOX_DATADEAL_S` and found it holding both IRS (2,096 rows) and CCS (904) at once — two independent confirmations of the same fact.

### GBO's `Financial Status`, for comparison `[confirmed: DB via BOX FE Developer, 2026-09-15]`

`GBO > Financial Engine > Financial Status` lists **every** product, with a per-product **triad of tables** — and it is structured differently from BOX's in ways that matter (full structural comparison and the product-set mapping in [`box-data-model.md`](box-data-model.md#financial-status-surfaces-gbo-vs-box-fe)):

- `Deal Data` → `DEVENG.T_PGT_<CODE>DATAMIS_S` — **one table per product**, unlike BOX's single shared `T_BOX_DATADEAL_S`.
- `Financial Data` → `DEVENG.T_PGT_<CODE>FINANCST_S` — per product, the one row that maps cleanly onto BOX.
- `Marketvalue and Risk` → `DEVENG.T_PGT_<CODE>RISK_S` — a **table**; BOX has no per-product `RISK` table at all.
- `MIS-MDR Interface` → `DEVENG.V_PGT_ENG<CODE>DATAMIS_S` — a view, present for some products only.

| GBO product | `<CODE>` | GBO product | `<CODE>` |
|---|---|---|---|
| Cap & Floor | `CF` | Futures | `FU` |
| CFM | **`IF`** | FX Spot | `SP` |
| Credit Derivatives | `CD` | FX ND Spot | `NDS` |
| CS (Currency Swap) | `SW` | FX Fwd (swap points) | `FP` |
| EQFW / EQLE / EQRP / EQSP | as written | FX Fwd (money market) | `FM` |
| ES | `ES` | Lending | `LE` |
| ETD | *(exception — see below)* | LoanDeposit | `MM` |
| FIFW | `FIFW` | OTC > OB / OE / OF / OI / OS | `OB`…`OS` |
| FISP | `FISP` | Repo | `RP` |
| FRAS (Forward Rate Agreement) | **`FR`** | SCF (Simple Cash Flow) | `SCF` |
| | | Swap | `IR` |

**⚠️ The same view name means different things on the two sides.** `V_PGT_ENG<CODE>DATAMIS_S` is labelled **MIS-MDR Interface** in GBO, while the identically-shaped `V_BOX_ENG<CODE>DATAMIS_S` is labelled **MtM Data** in BOX. Do not assume the `V_*_ENG<CODE>DATAMIS_S` object plays the same role on both sides just because the naming convention matches — GBO's MtM/risk equivalent is the separate `T_PGT_<CODE>RISK_S` table. `[open-question]` — whether these views are functionally equivalent, partially overlapping, or unrelated has not been established.

**Two GBO structural exceptions:**
- **ETD** breaks the triad entirely: no Deal Data object; two Financial Data **views** in a *different schema* (`DEVENG.V_TRD_FE_ETDPOS_S`, `DEVENG.V_TRD_FE_ETDIM_S` — note the `TRD` infix); Marketvalue and Risk from `PGT_MRK.T_FE_ETD_OPTMTM_S` (the shared market-data schema, not `DEVENG`).
- **SCF**'s Financial Data carries an `_OPTZ` suffix no other product has: `DEVENG.T_PGT_SCFFINANCST_S_OPTZ`. The same suffix appears elsewhere in this corpus (`T_BOX_BRPROCCAL_S`/`_QUEUE_S_OPTZ` in the onboarding checklist), so it looks like a shop-wide convention rather than an SCF quirk — meaning unconfirmed. `[open-question]`

**Coverage caveat:** the MIS-MDR Interface row was **not** listed for every GBO product by the person who supplied this. An absent MIS-MDR row above is therefore *unrecorded*, not evidence that the product lacks one. The product list itself was stated to be complete.

### GBO `Static MIS Data` tables `[confirmed: DB via BOX FE Developer, 2026-09-15]`

The `GBO > Financial Engine > Static MIS Data` folder (listed in the tree above, contents previously undocumented) has at least two leaves whose tables are now known:

| SIGOM path | Table | Note |
|---|---|---|
| `GBO > Financial Engine > Static MIS Data > Branch Mis` | `DEVENG.T_PGT_BRANCH_MIS_S` | Branch MIS ↔ Branch MDR mapping — **branch-keyed** |
| `GBO > Financial Engine > Static MIS Data > MISProduct` | `DEVENG.T_PGT_ENGMISPRODUCT_S` | MIS product catalogue |

**`T_PGT_BRANCH_MIS_S` is a lead worth chasing.** It is a *branch-keyed* GBO static-MIS table, and no equivalent appears anywhere in this repo's BOX FE config walk (`agents/sigom-box-fe-configs-agent/AGENT.md`'s 13 steps) or in [`branch-config/fe-branch-configuration.md`](branch-config/fe-branch-configuration.md). Either it has a BOX counterpart nobody has named yet, or branch↔MDR mapping is handled somewhere else in BOX entirely. Until that's answered it's an unquantified gap in the FE walk, not a confirmed omission — **do not add it to the walk on the strength of this observation alone.** `[open-question]`

**BOX's `Processed Instruments` and the Fixing-Exceptions view may be the same population.** `BOX - Financial Engine > Static IT Data > Processed Instruments` resolves to `BOX_FE.T_BOX_ENGINSTRUMENTS_S`, and its GBO twin is `DEVENG.T_PGT_ENGINSTRUMENTS_S` `[confirmed: DB via BOX FE Developer, 2026-09-15]`. This repo separately documents `BOX_FE.V_BOX_PROC_INSTR_S` — the "processed instrument" view that Fixing Exceptions joins to (`branch-config/fe-branch-configuration.md` §2, query Q-07 in [`queries/fe-config-mining.md`](queries/fe-config-mining.md)). A view named `PROC_INSTR` and a screen named `Processed Instruments` plausibly sit over the same catalogue, with the view being the table's read surface — but *plausibly* is the operative word: no query has been run joining or counting them against each other. `[open-question]` — cheap to settle on the first Tier 2 run, and worth settling, since Q-07's join depends on what that view actually contains. Note this is now a *three*-object question, not two: `T_BOX_ENGINSTRUMENTS_S` (table), `V_BOX_PROC_INSTR_S` (view) and their GBO counterparts.

### Codes/domain screens share one table `[confirmed: DB via BOX FE Developer, 2026-09-15]`

Several leaves under `Static IT Data` (BOX) / `Static MIS Data` (GBO) are **codes configuration** screens — `Deal Status` types being the example given — and they are **not** backed by one table each. They all read a single domain table:

| Side | Table |
|---|---|
| BOX | `BOX_FE.T_BOX_ENGDOM_S` |
| GBO | `DEVENG.T_PGT_ENGDOM_S` |

This is the same shape as the shared `T_BOX_DATADEAL_S` pattern in Financial Status: many screens, one underlying table, presumably discriminated by a domain/type column. Which specific leaves resolve here has not been enumerated — `Deal Status` is confirmed as one of them; `Event`, `Event Parameter`, `Exec Direction`, `Fee Type`, `Process`, `Process Status` (the other Static IT Data leaves listed in the tree above) are plausible but unconfirmed. `[open-question]`

> **⚠️ Schema/table names read as swapped in the source screenshot.** The supplied screenshot lists
> `DEVENG.T_PGT_ENGDOM_S` on the **BOX** line and `BOX_FE.T_BOX_ENGDOM_S` on the **GBO** line — the
> reverse of every other confirmed mapping in this corpus. Recorded above in the corrected orientation
> on three independent grounds: the companion screenshot supplied at the same time has Processed
> Instruments the correct way round (BOX→`BOX_FE`, GBO→`DEVENG`); the GBO↔BOX FE naming rule
> (`branch-config/fe-branch-configuration.md`) is confirmed across many tables; and
> [`box-data-model.md`](box-data-model.md) already listed `T_BOX_ENGDOM_S` under **BOX_FE**'s Financial
> Engine Configuration Tables before this screenshot existed. Treated as a transcription slip, not a
> genuine cross-wiring — but **worth one word of confirmation from the developer**, because if the
> schemas really are crossed for this table that would be a significant and deliberate exception.

**`T_BOX_ENGDOM_S` is not `PGT_SYS.PGT_DOMAINS`.** Two different generic "domain/enum" tables exist and must not be conflated: `T_BOX_ENGDOM_S` / `T_PGT_ENGDOM_S` are **module-owned** (one copy per side, FE-specific codes like Deal Status), while `PGT_SYS.PGT_DOMAINS` is **shared** across BOX and GBO and serves unrelated purposes (quote type, book label, folder coverage type/list — see [`box-data-model.md`](box-data-model.md)). Same concept, different tables, different ownership.
...
> ## 🔑 What the auth code actually is — resolved 2026-09-17
>
> `[confirmed: source via BOX FE Developer]` **The auth code is an environment-level constant, and it
> is the source of the `<integer>.<suffix>` PK format seen throughout this corpus.** The database
> function `F___SEQUENCE(TABLE_NAME, seq_range)` allocates every BOX FE primary key: it takes
> `NEXTVAL` from a shared sequence (`SQ_BOX_FINANENG1` for the branch-config tables) and, when
> `seq_range = 'X'`, adds `auth_code / 10^length(auth_code)` — where `auth_code` is read from the
> **single global row** in `gom_glb_sys.t__CORE_INFO_S`. So `21 → +0.21`, `4 → +0.4`, `65 → +0.65`,
> `35 → +0.35`. Full decode in
> [`queries/fe-config-mining.md`](queries/fe-config-mining.md), Q-G3.
>
> **This corrects how the corpus has been reading those suffixes.** They have been treated as
> *ownership* markers — `.21` = "Madrid-owned", `.4` = "global/shared", `.65` = "BOX-DEV/AUKI-owned".
> The causal story is different: a suffix records **which environment allocated the row**, not who owns
> it. The correlation with Madrid is real but incidental — Madrid's data was created in the instance
> whose auth code is 21. Two practical consequences:
>
> - **You cannot infer ownership from a suffix.** You can infer origin environment. A `.4` row in a
>   Tier 1 database was allocated somewhere with auth code 4 and arrived by replication or import,
>   which is why globally-shared static data (calendars, currencies) carries `.4` while locally-created
>   configuration carries `.21`. `[inferred]` — the coexistence explanation follows from the function
>   plus observation, and has not been confirmed by a developer.
> - **The status-bar label is a display convenience.** `21 - Madrid` pairs the numeric code with a
>   human name; the number is what the function uses. The earlier note below — that `Current auth-code`
>   looks session-level and global rather than per-screen — is **confirmed correct** by the function
>   reading it from one global row.
>
> Prior notes elsewhere in the repo calling `.65` "semantically load-bearing but unexplained"
> ([`../process/04-add-book-procedure.md`](../process/04-add-book-procedure.md),
> [`../process/05-add-product-procedure.md`](../process/05-add-product-procedure.md)) were right that it
> was load-bearing. It is now explained.
...
- **Numeric format correction (revises earlier documentation):** the `Group Id` column here shows values like `2.755,65`, `2.408,65`, `2.735,65` — clearly using a **period as thousands separator and a comma as decimal separator** (standard Spanish/European number formatting), e.g. `2.755,65` = 2755.65. This strongly suggests the `Internal Id` values documented on earlier screens (e.g. `895,21`, `1.399,21`, `4,65`) are **single decimal numbers in this same European format**, not two separate comma-joined figures as originally (and tentatively) hypothesized. The `Internal Id` open question about a "branch code" second component is very likely simply the decimal portion of a European-formatted number, not a meaningful separate code — this weakens that hypothesis further, close to resolving it as a documentation formatting artifact rather than a real substructure.
  - **⚠️ Half of the above was wrong, corrected 2026-09-16.** The European-format reading is right: `2.755,65` *is* `2755.65`, a single decimal number. The conclusion drawn from it — that the decimal part is therefore "not a meaningful separate code" — is **disproven**. The fractional part is the **auth-code**, the very same value this screen's own status bar displays as `Current auth-code: 21 - Madrid`, and it is relied on arithmetically by production delete guards (`trunc(PK) = sign(PK)*(abs(PK)-.65)`). So the digits after the separator are a real discriminator that happens to be *encoded* as a decimal fraction. See [`tables/t-box-cross-ref-s.md`](tables/t-box-cross-ref-s.md). Left in place rather than rewritten, because the formatting observation itself was correct and useful — only the inference from it was not.
...
- ~~Is `Current auth-code` a fixed numeric code per branch (e.g. Madrid = 21)?~~ **Resolved 2026-09-16 — yes, and it is the same number as the PK's decimal suffix.** `T_BOX_MBJ_PROPERTIES_S` records Madrid's branch as PK `22.21`, matching the `Current auth-code: 21 - Madrid` shown on every Madrid-filtered screen here. See [`tables/t-box-cross-ref-s.md`](tables/t-box-cross-ref-s.md) for the full three-suffix analysis (`.4` / `.21` / `.65`) and the ownership hypothesis. **Still open:** the codes for other branches — SLB, and Tier 2's NY/Brazil/Mexico. That list is now a directly answerable query rather than a mystery, and NY_SCH's code is a Phase 0 question.
