...
def main() -> int:
    ...
    expect("run-3-like output trips every expert-review check", build_fixtures.OUT / "run3-like",
           must_fail={"auth-code-not-asserted", "source-column-wrong", "quote-refs-not-evidence", "curve-fk-null",
    ...
    expect("config script reads DEVENG (applying account sees BOX_FE only)", mutated(good, "*config.sql",
           "IF v_quote_refs.COUNT <> c_expected_quote_refs THEN",
           "SELECT COUNT(*) INTO v_n FROM DEVENG.T_PGT_ENGLKFC_X;\n  IF v_quote_refs.COUNT <> c_expected_quote_refs THEN"),
           must_fail={"config-reads-other-schema"})
    expect("a quote ref that is not in the evidence", mutated(good, "*config.sql",
           "sys.odcinumberlist(6400.4,", "sys.odcinumberlist(6399.4,"), must_fail={"quote-refs-not-evidence"})
    expect("an exception row for another branch", mutated(good, "*config.sql",
           "SELECT 2.4 AS FK_INSTRUMENT, 142.4 AS FK_STRATEGY", "SELECT 2.4 AS FK_INSTRUMENT, 144.4 AS FK_STRATEGY"),
           must_fail={"exceptions-not-evidence"})
    ...
    expect("script built for another auth code", mutated(good, "*config.sql",
           "c_expected_fraction   CONSTANT NUMBER := 0.99;", "c_expected_fraction   CONSTANT NUMBER := 0.21;"),
           must_fail={"auth-code-mismatch"})
    ...
    expect("no PK check before the first INSERT", mutated(good, "*config.sql",
           "chk_pk(v_conf_pk,  'T_BOX_ENGCONF_S');\n  v_curve_pk := F___SEQUENCE('T_BOX_ENGFCURVE_S','X');  chk_pk(v_curve_pk, 'T_BOX_ENGFCURVE_S');",
           "\n  v_curve_pk := F___SEQUENCE('T_BOX_ENGFCURVE_S','X');"),
           must_fail={"auth-code-not-asserted"})

    # run-4 defects
    expect("cursor field not named by its SELECT (run 4 step 7)", mutated(good, "*config.sql",
           "r.FK_INSTRUMENT, r.FK_STRATEGY, r.FK_INSTRTYPE", "r.instrument, r.strategy, r.instrtype"),
           must_fail={"cursor-field-undeclared"})
    expect("Days Matured written with the Config owner (run 4 step 14a)", mutated(good, "*config.sql",
           "c_owner_days_matured  CONSTANT NUMBER := 35000145.65;", "c_owner_days_matured  CONSTANT NUMBER := 35000126.65;"),
           must_fail={"identity-wrong-object"})
    expect("dummy book set to the real book (run 4 step 11)", mutated(good, "*config.sql",
           "c_dummy_book          CONSTANT NUMBER := 26391.4;", "c_dummy_book          CONSTANT NUMBER := 23958.44;"),
           must_fail={"dummy-is-a-book"})
    expect("verify check that cannot fail (run 4 V1)", mutated(good, "*verify.sql",
           "-- V2 — ROW COUNTS", "SELECT COUNT(*) FROM BOX_FE.T_BOX_ENGCONF_S WHERE 1=0 AND 0.44 <> 0.44;\n-- V2 — ROW COUNTS"),
           must_fail={"verify-placebo"})
    expect("verify without the GBO comparison for step 4", mutated(good, "*verify.sql",
           "DEVENG.T_PGT_ENGLKFC_X", "BOX_FE.T_BOX_ENGLKFC_X", 2), must_fail={"verify-incomplete"})
    expect("step header without its Source line (run 4)", mutated(good, "*config.sql",
           "  -- Source      : FK_BS = BRANCH_PK (Q-G1)\n", ""), must_fail={"step-header-incomplete"})
    expect("no curve pre-commit (run 4)", mutated(good, "*config.sql",
           "PKG_ENGPRECOMMIT.P_ENGFixingCurve_PreCommit(v_curve_pk);", "NULL;"),
           must_fail={"curve-precommit-missing"})
    tagged = mutated(good, "*config.sql", "SET DEFINE OFF", "SET DEFINE OFF")
    fnd = tagged / "02-findings.md"
    fnd.write_text(fnd.read_text() + "\nStep 6: [confirmed: Q-05d] SME approved SLB BOX values\n")
    expect("a decision tagged [confirmed] (run 4, step 6)", tagged, must_fail={"decision-tagged-confirmed"})
    nodec = mutated(good, "*config.sql", "SET DEFINE OFF", "SET DEFINE OFF")
    (nodec / "00-decisions.md").unlink()
    expect("decided steps with no decisions log", nodec, must_fail={"decisions-log-missing"})
