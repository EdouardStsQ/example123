...
def plsql_lint(sql: str) -> list[str]:
    ...
    for b in blocks:
        ...
        n_end = len(re.findall(r"\bEND\b(?!\s+(IF|LOOP)\b)", up))
        if n_if != n_end_if:
...
def check_rendered_files(label: str, res) -> None:
    for name, text in res.files.items():
        ...
        if not name.endswith("verify.sql"):
            probs = plsql_lint(text)
            report(not probs, f"{label}: {name} passes the PL/SQL structural lint", "; ".join(probs))
...
def main() -> int:
    ...
    report("p_check_Val_Curves_precommit" in code_only(cfg) and "ROLLBACK;" not in code_only(cfg)
           and "ROLLBACK;" in code_only(reh) and "p_check_Val_Curves_precommit" not in code_only(reh),
           "config ends in the committing pre-commit, rehearsal in ROLLBACK and never calls it")
    ...
    expect("no curve pre-commit (run 4)", edited(C, "BOX_FE.PKG_ENGPRECOMMIT.P_ENGFixingCurve_PreCommit(v_curve_pk);", "NULL;"),
    ...
    expect("Config pre-commit replaced by COMMIT (run 5)", edited(C,
           "BOX_FE.PKG_ENGPRECOMMIT.p_check_Val_Curves_precommit(v_conf_pk);", "COMMIT;"), {E, "config-precommit-missing"})
    early = edited(C, "  -- curve validation, after its array exists - no DML, safe as a gate (read 2026-09-21)\n"
