...
## ⛔ The validator is a gate, not a report — iterate until it is clean

**Neither previous run pasted validator output.** Both were rejected for defects it now catches. So:

1. Run `python3 scripts/validate_run_output.py runs/NY_SCH/tier2-pre/`.
2. **Fix every FAIL.**
3. Run it again.
4. Repeat until it reports **0 FAIL**.
5. **Paste the final clean output as the last thing you produce.**

**Do not present the run as finished until step 4 passes.** A FAIL is a defect regardless of how the
write-up reads, and every check encodes a defect this project has already produced — the second time
would be a choice.

If a check looks wrong to you, **say so and leave it failing.** Do not reshape the SQL to slip past a
check: run 2 replaced `WHERE ROWNUM = 1` with `GROUP BY … HAVING`, which changed the spelling and kept
the defect. A check you disagree with is a finding about the check.
