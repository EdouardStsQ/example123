...
| 0d | **Q-G3b** (a)–(d) | `F___SEQUENCE` not visible or not `VALID` → stop. **`PKG_ENGPRECOMMIT` returning 0 rows is NOT a stop** — the read-only account is simply not granted it (run 6, 2026-09-24): record owner `BOX_FE` `[confirmed: source, cib-boxfin-dbboxfe 023-pkg_engprecommit_body.sql]` and continue; the rehearsal calls it and reports a `WARNING` if the applying account cannot. **Record both owners** — `values.json` `environment` |
| 0d | 🆕 **Q-G3c** | The target's auth code. `core_info_rows ≠ 1` → stop. **Record `TARGET_AUTH_CODE` and `TARGET_PK_FRACTION` in `00-inputs.md`** |
| 0d | 🆕 **Q-G3d** (c)(e), and (b) if visible | (a) is **answered from the source** (see Q-G3d's box: no transaction control in `F___SEQUENCE` or the curve pre-commit; the Config pre-commit commits only while back-filling step-8 rows) — do not ask it again. **(e) → `01-evidence/Q-G3d-columns.csv`**: the renderer checks every column against it. A trigger in (b) that writes elsewhere → source question |
| 0d | 🆕 **Q-G3e** | Sequence headroom: is any PK with this environment's fraction ahead of `SQ_BOX_FINANENG1`? Tier 2 may hold typed PKs `[stated: BOX Dev, 2026-09-24]`. Yes → a question to the BOX FE team before the rehearsal (the block's `chk_pk` would stop it, keeping nothing). Never type PKs |
...
2. **Owners** — `environment.sequence_function_owner` from **Q-G3b (b)**; `precommit_package_owner`
   from Q-G3b (d), or — when (d) returns 0 rows on the read-only account — `BOX_FE` from the source.
...
pre-commit after the self-check (both called dynamically: not callable → `WARNING`, never a stop), then
the block's own `COMMIT` (rehearsal: `ROLLBACK`); every step's full header; `BOX_FE` only; ASCII only; the verify file V1–V12; the
