...
| 7 | **Q-06 — the expert's query** | Returns every branch's exceptions under `64408.35`, with the branch named |
...
`-rollback.sql`. Replace every placeholder with the recorded value **and its source**. No `&&`, no
leftover placeholder, no `IF 1 = 0`.

1. 🆕 **Auth code — visible and asserted** (point 1). The config script reads the auth code first and
   aborts if it is not `TARGET_AUTH_CODE`; `chk_pk()` after **every** `F___SEQUENCE`; verify **V1** checks
   every row created. **Never type the fraction** (the team's own `F___SEQUENCE(…,1) + 0.21` is not the
   pattern — it is right in one environment only).
...
3. 🆕 **Step 4 is read, not typed** (point 3). Cursor over `DEVENG.T_PGT_ENGLKFC_X WHERE FK_PARENT = <GBO
   curve>`, on the column **Q-04c** proved, count-guarded against Q-04c (b), verify **V4** both ways. In
   the findings, **say which column run 3's 38 values came from** — the expert doubted them.
4. 🆕 **Step 7 — NY's rows, written with NY's branch** (point 4). From Q-06: keep rows whose branch
   ...
   `CRITERIAL` from the GBO row. Read in-script with a count guard (template).
...
whether the account that applies the script can read `DEVENG` (steps 4 and 7 need it; if not, the block
fails to compile and nothing is inserted); Q-04c's outcome if it is B or C; what `DDATE` means on Days
Matured.
