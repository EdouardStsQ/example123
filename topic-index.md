...
| `reference/job-chains/box-fe-acc-batch-runtime.md` | FE — the internal FE batch: nine event groups in order, with PKs and per-procedure execution order | DataLake *(picks up where the RAW load ends)*, Accept, ACC *(pending — ACC half not yet written)* |
