...
| `reference/book-and-folder.md` | **Gate** — the two dimensions a trade carries: BOOK (the desk, `PGT_DOMAINS` via `FK_OWNER_OBJ = 17910.4`) and FOLDER (an aggregation of Murex portfolios, owned by GER). Why processing partitions on (Book, Instrument, Branch), and the missing mapping between them | FE, ACC, DataLake, Accept |
...
| `decisions/` | *(cross-reference — no primary)* — **architecture decision records.** Where a withdrawn mode's reasoning lives once it leaves a charter: 0001 split-tier runs, 0002 deferred-PK mode, 0003 gate 0f. Each names what was kept in the charter, so a cut can be audited | FE |
| ⚠️ `reference/job-chains/box-fe-acc-batch-runtime.md` — **this file does not exist** and is linked from four documents. Either write it or fix the links | FE + ACC — the **scheduler** layer: Control-M job families, the `T_BOX_MBJ_PROPERTIES_S` job→(branch, instrument, MBJ group) binding, and the FE→ACC handoff through Control-M events | DataLake *(picks up where the RAW load ends)*, Accept, Online *(one job carries `MIC_FLOW_MODE=On-Line` — an unexplained touchpoint)* |
