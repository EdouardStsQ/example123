...
def check_sql(path: Path, raw: str, rpt: Report) -> None:
    ...
    # --- identity columns come from Q-G6, not from an arbitrary row -----
    if re.search(r"select[\s\S]{0,200}?fk_owner_obj[\s\S]{0,200}?rownum\s*=\s*1", low):
        rpt.fail("identity-from-rownum",
                 "FK_OWNER_OBJ / FK_EXTENSION sourced with WHERE ROWNUM = 1 — that is an "
                 "arbitrary existing row, not the declared identity. Resolve them from "
                 "Q-G6 against the target and emit the constants (hard rule 9)", name)
