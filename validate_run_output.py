...
def check_sql(path: Path, raw: str, rpt: Report) -> None:
    ...
    if re.search(r"insert\s+into\s+[\w.]*t_box_engfcurve_s", low):
        ...
        if not linked:
            rpt.fail("orphan-curve",
                     "a curve is INSERTed into T_BOX_ENGFCURVE_S but no UPDATE of "
                     "T_BOX_ENGCONF_S sets FK_CURVEMAN/FK_CURVEACC to its variable — "
                     "the configuration points at no curve this run created "
                     "(steps 2–4 are insert-then-update)", name)
    ...
    for m in re.finditer(r"select\b[\s\S]{0,300}?\binto\b[\s\S]{0,300}?;", low):
        frag = m.group(0)
    ...
    # Run 2 declared a quote-reference array and emitted one row of 38. No
    # rule can know 38 is right; the file can be made to agree with itself.
    for m in re.finditer(r"^[ \t]*--[ \t]*step[ \t]+(\d+[a-z]?)\b", raw, re.M | re.I):
        step = m.group(1)
        # The header block is the comment run that follows, capped so a count
        # mentioned far down in the body is not mistaken for the step's own.
        block = raw[m.end(): m.end() + 500]
        table = TARGET_TABLE_BY_STEP.get(step.lower())
        if not table:
            continue
        cm = re.search(r"(\d+)\s*rows?\b", block, re.I)
        if not cm:
        ...
        if actual != declared:
            rpt.fail("step-row-count-mismatch",
                     ...
                     f"emits {actual}. A set that shrinks between the finding and the "
    ...
    if sql_dir.is_dir():
        ...
        if findings.exists() and sql_files:
            ...
            for line in ftext.splitlines():
                ...
                if ready and table.upper() not in all_sql:
