...
def check_run4_rules(name: str, code: str, inserts: list[str], raw: str, rpt: Report) -> None:
    ...
    for step, block in step_blocks(raw):
        if step in {"11", "12", "14a"}:
            blow = strip_sql_comments(block).lower()
            if re.search(r"\binsert\s+into\b", blow) and "raise_application_error" not in blow:
                rpt.fail("no-existing-row-guard",
                         f"step {step} inserts without first aborting on existing rows — the rollback finds "
                         "these rows by branch or instrument, so it may delete rows this run did not create",
                         name)
...
def check_verify_file(name: str, raw: str, config_code_low: str, rpt: Report) -> None:
    ...
    missing = []
    if "gom_glb_sys.t__core_info_s" not in low:
