...
def check_expert_review_rules(name: str, raw: str, sql: str, low: str,
    ...
    if "t__core_info_s" not in low:
        rpt.fail("auth-code-not-asserted",
                 "the config script never reads gom_glb_sys.t__CORE_INFO_S — the auth code its PKs "
                 "carry is invisible and unchecked. Assert it before the first INSERT (Q-G3c, "
                 "hard rule 3)", name)
    elif n_chk < n_alloc:
    ...
    core = low.find("t__core_info_s")
    if core >= 0 and first_insert and core > first_insert.start():
        rpt.fail("auth-code-not-asserted",
                 "the auth code is read only after the first INSERT — the check must stop a script "
                 "in the wrong environment before it writes anything", name)
    want = RUN_INPUTS.get("TARGET_AUTH_CODE")
    if want and core >= 0 and not re.search(
            rf"c_expected_auth_code\s+constant\s+number\s*:=\s*{re.escape(want)}\s*;", low):
        rpt.fail("auth-code-mismatch",
                 f"00-inputs.md records TARGET_AUTH_CODE {want} but the script's expected auth-code "
                 "constant is not that value", name)
    ...
    # --- point 3: the quote array is read, not typed ------------------------
    if any(re.search(r"t_box_englkfc_x", s, re.I) for s in inserts) and \
            "deveng.t_pgt_englkfc_x" not in low:
        rpt.fail("quote-refs-typed",
                 "step 4 INSERTs quote references but never reads DEVENG.T_PGT_ENGLKFC_X — a typed "
                 "list cannot be checked against its source. Read the set in-script (Q-04c)", name)
...
def check_sql(path: Path, raw: str, rpt: Report) -> None:
    ...
    if inserts:
        check_expert_review_rules(name, raw, sql, low, inserts, rpt)
