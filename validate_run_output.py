...
# A CALL of each pre-commit, direct or inside EXECUTE IMMEDIATE '…' (package-qualified, followed by "("):
# a message that merely names the procedure is not a call.
CALL_CURVE_PC = re.compile(r"\.\s*p_engfixingcurve_precommit\s*\(", re.I)
CALL_CONF_PC = re.compile(r"\.\s*p_check_val_curves_precommit\s*\(", re.I)
...
def check_run4_rules(name: str, code: str, inserts: list[str], raw: str, rpt: Report) -> None:
    ...
    if re.search(r"insert\s+into\s+[\w.]*t_box_engfcurve_s", low) and not CALL_CURVE_PC.search(low):
...
def check_run5_rules(name: str, code: str, inserts: list[str], raw: str, rpt: Report) -> None:
    ...
    if re.search(r"insert\s+into\s+[\w.]*t_box_engconf_s\b", low) and not CALL_CONF_PC.search(low):
    ...
    pcm = CALL_CURVE_PC.search(low)
    pc = pcm.start() if pcm else -1
